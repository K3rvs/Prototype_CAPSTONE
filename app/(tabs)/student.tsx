...
export default function StudentDashboard() {
  ...
  const handleGoBackPassport = useCallback(() => {
    setMountedTabs(prev => new Set(prev).add(previousTab === "passport" ? "dashboard" : previousTab));
  ...
  const memoizedDashboard = useMemo(() => <StudentDashboardSection onNavigate={handleNavigate} />, [handleNavigate]);
  const memoizedClasses = useMemo(() => <StudentClasses onTestActiveChange={setIsTestActive} selectedClassId={selectedClass} onClassSelect={setSelectedClass} />, [selectedClass]);
  const memoizedCommunities = useMemo(() => <StudentCommunities />, []);
  const memoizedPassport = useMemo(() => <StudentAcademicPassport goBack={handleGoBackPassport} />, [handleGoBackPassport]);
  const memoizedCalendar = useMemo(() => <StudentSchoolCalendar />, []);
  const memoizedFAQ = useMemo(() => <StudentHelpFAQ />, []);
  const memoizedProfile = useMemo(() => <StudentProfileSection goBack={handleGoBackProfile} />, [handleGoBackProfile]);
        ...
        <View className={isMobileMenu ? "gap-3" : "gap-2"}>
          {TABS.map((tab) => (
            <Pressable key={tab.id} onPress={() => { 
              ...
              <MaterialCommunityIcons name={tab.icon as any} size={isMobileMenu ? 28 : 20} color={activeTab === tab.id ? "#0284c7" : "#64748b"} />
              <Text className={`font-bold ${isMobileMenu ? "text-lg" : "text-sm"} ${activeTab === tab.id ? "text-sky-900" : "text-slate-600"}`}>{tab.label}</Text>
            </Pressable>
          ))}
        </View>
      </ScrollView>
  ...
  const renderMoreMenu = () => (
    <ScrollView className="flex-1 px-4 py-6 bg-slate-50" showsVerticalScrollIndicator={false}>
      <View className="mb-6">
        <Text className="text-2xl font-black text-slate-900 tracking-tight">More Options</Text>
        <Text className="text-slate-500 text-xs sm:text-sm mt-1 font-medium">Explore additional features and account settings</Text>
      </View>

      <View className="gap-4 pb-12">
        <Pressable
          onPress={() => { setPreviousTab("more"); setMountedTabs(prev => new Set(prev).add("calendar")); setActiveTab("calendar"); }}
        ...
        >
          <View className="flex-row items-center gap-4 flex-1">
            <View className="w-12 h-12 bg-amber-50 rounded-xl items-center justify-center border border-amber-100">
              <MaterialCommunityIcons name="calendar-month-outline" size={24} color="#d97706" />
            </View>
            <View className="flex-1">
              <Text className="font-bold text-slate-800 text-base">Calendar</Text>
              <Text className="text-slate-500 text-xs mt-0.5">View school events and calendars</Text>
            </View>
          </View>
        ...
        <Pressable
          onPress={() => { setPreviousTab("more"); setMountedTabs(prev => new Set(prev).add("faq")); setActiveTab("faq"); }}
          className="flex-row items-center justify-between p-4 bg-white rounded-2xl border border-slate-200 shadow-sm active:bg-slate-50"
        >
          <View className="flex-row items-center gap-4 flex-1">
            <View className="w-12 h-12 bg-sky-50 rounded-xl items-center justify-center border border-sky-100">
              <MaterialCommunityIcons name="help-circle-outline" size={24} color="#0284c7" />
            </View>
            <View className="flex-1">
              <Text className="font-bold text-slate-800 text-base">Help & FAQ</Text>
              <Text className="text-slate-500 text-xs mt-0.5">Find answers to frequently asked questions</Text>
            </View>
          </View>
          <MaterialCommunityIcons name="chevron-right" size={22} color="#cbd5e1" />
        </Pressable>
        ...
        <Pressable
          onPress={() => { setPreviousTab("more"); setMountedTabs(prev => new Set(prev).add("profile")); setActiveTab("profile"); }}
          className="flex-row items-center justify-between p-4 bg-white rounded-2xl border border-slate-200 shadow-sm active:bg-slate-50"
        >
          <View className="flex-row items-center gap-4 flex-1">
            <View className="w-12 h-12 bg-indigo-50 rounded-xl items-center justify-center border border-indigo-100">
              <MaterialCommunityIcons name="account-outline" size={24} color="#4f46e5" />
            </View>
            <View className="flex-1">
              <Text className="font-bold text-slate-800 text-base">Profile & Settings</Text>
              <Text className="text-slate-500 text-xs mt-0.5">View and update your personal details</Text>
            </View>
          </View>
          <MaterialCommunityIcons name="chevron-right" size={22} color="#cbd5e1" />
        </Pressable>
      ...
      <SafeAreaView className="flex-1 bg-white relative flex-col">
        ...
        {!isTestActive && (
          ...
          <View className="flex-1 flex-row items-center justify-start">
            <Pressable onPress={() => setIsSidebarOpen(true)} className="items-center justify-center p-2 -ml-2 rounded-full active:bg-slate-100">
          ...
          <View className="flex-row items-center justify-center gap-2 shrink-0">
            ...
            <Text className="text-xl font-black text-slate-900 tracking-wider">i++</Text>
          </View>
          ...
          <View className="flex-1 flex-row items-center justify-end gap-3 sm:gap-4">
            ...
            <Pressable onPress={() => setIsNotifPageOpen(true)} className="relative">
              <MaterialCommunityIcons name={unreadCount > 0 ? "bell" : "bell-outline"} size={26} color="#0f172a" />
              {unreadCount > 0 && (
                <View className="absolute -top-1 -right-1 min-w-[16px] h-[16px] bg-red-500 rounded-full items-center justify-center border-2 border-white">
                  <Text className="text-[9px] font-bold text-white leading-none">{unreadCount > 9 ? '9+' : unreadCount}</Text>
                </View>
              )}
            </Pressable>
            <Pressable onPress={() => { setPreviousTab(activeTab); setMountedTabs(prev => new Set(prev).add("profile")); setActiveTab('profile'); }} className="w-8 h-8 bg-sky-600 rounded-full items-center justify-center ml-1">
        ...
        <Modal visible={isSidebarOpen} animationType="fade" transparent={true} onRequestClose={() => setIsSidebarOpen(false)}>
          <View className="flex-1 flex-row justify-start">
            {/* Backdrop */}
            <Pressable className="absolute inset-0 bg-slate-900/40" onPress={() => setIsSidebarOpen(false)} />
            {/* Sidebar Container */}
            <Animated.View 
              style={[
            ...
            >
              <SafeAreaView className="flex-1 bg-white shadow-2xl" edges={['top', 'bottom']}>
            ...
            <View className="flex-1 w-full bg-white">
              <View className="h-16 border-b border-slate-200 flex-row items-center px-4 gap-3">
                ...
                <View className="flex-1 bg-slate-100 rounded-full flex-row items-center px-4 h-11 border border-slate-200">
                  <TextInput placeholder="Search..." value={searchQuery} onChangeText={setSearchQuery} className="flex-1 text-slate-900 h-full" />
                </View>
              </View>
            ...
            <View className="h-16 border-b border-slate-200 flex-row items-center justify-between px-4">
              ...
              {unreadCount > 0 && (
                <Pressable onPress={markAllAsRead}>
                  <Text className="text-sky-600 font-bold text-sm">Mark All Read</Text>
                </Pressable>
              )}
            </View>
            <ScrollView className="flex-1 bg-white">
              {sortedNotifications.map(notif => (
                <Pressable key={notif.id} onPress={() => {
                  ...
                  <View className="flex-1">
                    <Text className={`text-base ${notif.read ? 'text-slate-700' : 'text-slate-900 font-bold'}`}>{notif.title}</Text>
      ...
      <View className="flex-1 relative z-10">
        ...
        {!isTestActive && (
          ...
          <View className="flex-row items-center gap-1 sm:gap-4">
            ...
            <Pressable
              onPress={() => { setPreviousTab(activeTab); startTransition(() => { setMountedTabs(prev => new Set(prev).add("profile")); setActiveTab("profile"); }); }}
        ...
        <Animated.ScrollView
          ...
          contentContainerStyle={{ paddingTop: isTestActive ? 0 : 76, paddingBottom: isTestActive ? 0 : 40, flexGrow: 1 }}
          showsVerticalScrollIndicator={false}
        >
          <Animated.View className="w-full max-w-7xl mx-auto flex-1">
            <View className="w-full mb-12 flex-1 relative">
              <View style={{ display: activeTab === "dashboard" ? "flex" : "none", flex: 1, ...(Platform.OS === "web" ? { zoom: 0.9 } : {}) } as any}>{mountedTabs.has("dashboard") && memoizedDashboard}</View>
              <View style={{ display: activeTab === "classes" ? "flex" : "none", flex: 1, ...(Platform.OS === "web" ? { zoom: 0.9 } : {}) } as any}>{mountedTabs.has("classes") && memoizedClasses}</View>
              <View style={{ display: activeTab === "communities" ? "flex" : "none", flex: 1, ...(Platform.OS === "web" ? { zoom: 0.9 } : {}) } as any}>{mountedTabs.has("communities") && memoizedCommunities}</View>
              <View style={{ display: activeTab === "passport" ? "flex" : "none", flex: 1, ...(Platform.OS === "web" ? { zoom: 0.9 } : {}) } as any}>{mountedTabs.has("passport") && memoizedPassport}</View>
              <View style={{ display: activeTab === "calendar" ? "flex" : "none", flex: 1, ...(Platform.OS === "web" ? { zoom: 0.9 } : {}) } as any}>{mountedTabs.has("calendar") && memoizedCalendar}</View>
              <View style={{ display: activeTab === "faq" ? "flex" : "none", flex: 1, ...(Platform.OS === "web" ? { zoom: 0.9 } : {}) } as any}>{mountedTabs.has("faq") && memoizedFAQ}</View>
              <View style={{ display: activeTab === "profile" ? "flex" : "none", flex: 1 }}>{mountedTabs.has("profile") && memoizedProfile}</View>
            </View>
          </Animated.View>
        </Animated.ScrollView>
      </View>
                    ...
                    return (
                      ...
                      >
                        ...
                        <View className="flex-1 justify-center">
                          ...
                          <Text className={`text-[10px] font-bold uppercase tracking-wider ${notif.read ? "text-slate-400" : "text-orange-500"}`}>
                            {notif.time}
                          </Text>
                        </View>
