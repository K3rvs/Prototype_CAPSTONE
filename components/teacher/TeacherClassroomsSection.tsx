...
import { Pressable, Text, TextInput, View, Platform, ScrollView } from "react-native";
import Animated, { FadeIn, FadeInDown } from "react-native-reanimated";
import { GlobalModal } from "@/components/shared/AdminUI";

export const TeacherClassroomsSection = ({ onSelectClass, classes, setClasses }: any) => {
    ...
    >
      ...
      <View className="flex-row flex-wrap gap-4 sm:gap-6 mt-4">
        {classes.map((cls: any, i: number) => (
          ...
          >
            <Pressable onPress={() => onSelectClass(cls)} className={`bg-white rounded-3xl shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all overflow-hidden active:scale-[0.97] flex-col min-h-[220px] group border border-slate-100 hover:border-${cls.color}-300 relative`}>
              ...
              <View className={`bg-${cls.color}-600 p-5 relative h-32 flex-col justify-between overflow-hidden`}>
                ...
                <View className="z-10 h-full flex-col justify-between">
                  <View className="flex-row items-start justify-between">
                    ...
                    >
                      <MaterialCommunityIcons name="archive-outline" size={16} color="white" />
                    </Pressable>
                  </View>
              ...
              <View className="p-5 flex-1 flex-col justify-between relative z-10 bg-white">
                ...
                <View className="mt-1 pr-14">
                  <Text className="text-slate-800 text-base font-black mb-2">My Class</Text>
                ...
                <View className="absolute bottom-0 left-0 right-0 h-8 flex-row items-end gap-[2px] px-5 opacity-20 pointer-events-none">
                   {cls.engagement.map((val: any, idx: number) => (
                      <View key={idx} className={`flex-1 bg-${cls.color}-400 rounded-t-sm`} style={{ height: `${val}%` }} />
                   ))}
                </View>
              </View>
            </Pressable>
        ...
        >
          ...
          >
            <View className="w-12 h-12 bg-white rounded-full items-center justify-center shadow-sm mb-3 group-hover:scale-110 transition-transform group-hover:bg-teal-100 border border-slate-200 group-hover:border-teal-200">
              <MaterialCommunityIcons name="plus" size={28} color="#0f766e" />
            </View>
      ...
      >
        ...
        <View className="flex-row gap-3">
          ...
          <Pressable
            ...
            onPress={() => {
              setClasses(classes.filter((c: any) => c.name !== targetArchive));
              setArchiveModal(false);
              setArchiveConfirmText("");
            }}
