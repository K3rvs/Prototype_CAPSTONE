  ...
  if (Platform.OS !== "web") {
    chartView = (
      <View
        ...
        className="shadow-md items-center justify-center bg-slate-100"
      >
    ...
    if (data.every((d) => d.percent === 0)) {
      chartView = (
        <View
          ...
          className="items-center justify-center shadow-md border-[6px] border-white bg-slate-200"
        >
          {innerContent}
        </View>
      );
      ...
      chartView = (
        <View
          ...
          className="items-center justify-center shadow-md border-[6px] border-white bg-slate-100"
        >
          {innerContent}
        </View>
      );
...
export const TeacherRemediationHub = ({
  ...
  let totalMaterials = 0,
    unstudiedMaterials = 0;
  let totalAI = 0,
    activeAI = 0;

  classes.forEach((cls: any) => {
    if (cls.assessments) {
      totalAssessments += cls.assessments.total;
      goodAssessments += cls.assessments.goodPerformance;
    }
    if (cls.materials) {
      totalMaterials += cls.materials.total;
      unstudiedMaterials += cls.materials.unstudied;
    }
    if (cls.ai) {
      totalAI += cls.ai.total;
      activeAI += cls.ai.active;
    }
  });

  const riskPercent =
    totalAssessments > 0
      ? Math.round((goodAssessments / totalAssessments) * 100)
      : 0;
  const materialsPercent =
    totalMaterials > 0
      ? Math.round((unstudiedMaterials / totalMaterials) * 100)
      : 0;
  ...
  const computedInsights = classes.map((cls: any) => {
    const avgEngagement =
      cls.engagement && cls.engagement.length > 0
        ? cls.engagement.reduce((a: number, b: number) => a + b, 0) /
          cls.engagement.length
        : 0;
    ...
    return {
      ...
      iconBg: `bg-${color}-200`,
      btnBg:
        priority === "High Priority" ? `bg-${color}-600` : `bg-${color}-50`,
      btnText:
        priority === "High Priority" ? "text-white" : `text-${color}-800`,
      btnActive:
        priority === "High Priority"
          ? `active:bg-${color}-700`
          : `active:bg-${color}-100`,
  ...
  const highCount = computedInsights.filter(
    (c: any) => c.priority === "High Priority",
  ).length;
  const mediumCount = computedInsights.filter(
    (c: any) => c.priority === "Medium Priority",
  ).length;
  const overallHealth = classes.length
    ? Math.round(
        computedInsights.reduce((acc, c) => acc + c.avgEngagement, 0) /
          classes.length,
      )
    : 0;
  ...
  const REMEDIATION_STATS = [
    ...
    },
    {
      ...
      value: (highCount + mediumCount).toString(),
      ...
      >
        <Animated.View entering={FadeIn} className="w-full">
          <View className="flex-col sm:flex-row justify-between sm:items-center mb-6 gap-4">
            ...
            <Pressable
              className="bg-white border border-slate-200 px-4 py-2.5 rounded-xl shadow-sm flex-row items-center gap-2 active:bg-slate-50 transition-colors"
              onPress={() => alert("Generating Global Report...")}
            >
              <MaterialCommunityIcons
                ...
                color="#0f766e"
              />
              <Text className="text-teal-800 font-bold text-sm">
                Global Report
              </Text>
            </Pressable>
          </View>
          ...
          <View className="flex-row flex-wrap gap-4 mb-6">
            {REMEDIATION_STATS.map((stat, i) => (
              <Animated.View
                ...
                className="w-[calc(50%-8px)] md:w-[calc(25%-12px)] bg-white p-4 rounded-3xl border border-slate-200 shadow-sm flex-col justify-between"
              >
                <View className="flex-row items-center justify-between mb-3">
                  <View
                    className={`w-8 h-8 rounded-full bg-${stat.color}-50 items-center justify-center border border-${stat.color}-100`}
                  >
                    <MaterialCommunityIcons
                  ...
                  <View
                    className={`px-2 py-0.5 rounded flex-row items-center gap-1 ${stat.trendUp ? "bg-emerald-50" : "bg-rose-50"}`}
                  >
                    <MaterialCommunityIcons
                    ...
                    />
                    <Text
                      className={`text-[10px] font-bold ${stat.trendUp ? "text-emerald-700" : "text-rose-700"}`}
                    >
                      {stat.trend}
                    </Text>
                  </View>
                </View>
                <Text className="text-3xl font-black text-slate-800 mb-1 tracking-tighter">
                  {stat.value}
                </Text>
          ...
          <View className="flex-col lg:flex-row gap-6 mb-6">
            ...
            <Animated.View
              ...
              className="w-full lg:w-[45%] bg-white p-5 rounded-3xl border border-slate-200 shadow-sm flex-col items-center"
            >
              <Text className="text-base font-black text-slate-800 self-start mb-4">
                Class Priority Distribution
              </Text>
              <Text className="text-slate-500 text-xs self-start mb-6 -mt-3">
                Click on any graph to view detailed breakdown per class.
              </Text>
                  ...
                  <PieChart
                    ...
                    onPress={() =>
                      setGraphModal({
                        ...
                        type: "assessments",
                        desc: "Based on students with good performance in assessments.",
                      })
                    }
                  />
                ...
                <View className="flex-row justify-around w-full mt-2">
                  ...
                  <View className="items-center gap-3">
                    <PieChart
                      ...
                      onPress={() =>
                        setGraphModal({
                          title: "Materials Engagement",
                          type: "materials",
                          desc: "Based on the number of students who haven't studied uploaded materials.",
                        })
                      }
                    />
                    <Text className="font-bold text-slate-700 text-sm">
                    ...
                    <PieChart
                      ...
                      onPress={() =>
                        setGraphModal({
                          title: "Classroom AI",
                          type: "ai",
                          desc: "Based on the number of students who are active in Classroom AI.",
                        })
                      }
                    />
                    <Text className="font-bold text-slate-700 text-sm">
            ...
            <View className="w-full lg:w-[calc(55%-24px)] flex-col">
              <View className="flex-row flex-wrap sm:flex-nowrap items-center justify-between mb-4 gap-2">
                ...
                <View className="flex-row flex-wrap gap-2">
                  {["All", "High Priority", "Medium Priority", "Resolved"].map(
                    (f) => (
                      <Pressable
                        key={f}
                        onPress={() => setFilter(f)}
              ...
              >
                <View className="flex-row flex-wrap gap-4 pb-4">
                  {computedInsights
                    .filter((ci) => filter === "All" || filter === ci.priority)
                    .map((insight, idx) => (
                      <Animated.View
                        key={insight.id}
                      ...
                      >
                        <Pressable
                          className={`${insight.bg} border ${insight.border} p-4 rounded-2xl shadow-sm w-full flex-col`}
                        >
                          <View className="flex-row items-start gap-4">
                            <View
                              className={`${insight.iconBg} w-10 h-10 rounded-full items-center justify-center border ${insight.border} shrink-0`}
                            >
                              <MaterialCommunityIcons
                                name={insight.icon as any}
                                size={20}
                                color={getIconColor(insight.color)}
                              />
                            </View>
                            ...
                            <View className="flex-1">
                              <View className="flex-row items-center justify-between mb-0.5">
                                <Text
                                  className={`${insight.text} font-black text-base tracking-tight`}
                                  numberOfLines={1}
                                >
                                  {insight.class}
                                </Text>
                                <View className="bg-white/60 px-2 py-0.5 rounded-md border border-white/80">
                                  <Text
                                    className={`${insight.text} font-bold text-[10px] uppercase tracking-widest`}
                                  >
                                    {insight.section}
                                  </Text>
                                </View>
                              ...
                              <Text
                                className={`${insight.text} opacity-80 font-bold text-[10px] mb-1.5 uppercase tracking-widest`}
                              >
                                {insight.priority}: {insight.topic}
                              </Text>
                              <Text
                                className={`text-${insight.color}-800/80 text-xs font-medium leading-snug`}
                                numberOfLines={2}
                              >
                                {insight.trigger}
                              </Text>
                              ...
                              <View className="flex-row gap-2 mt-3">
                                <Pressable
                                  onPress={() => {
                                    if (
                                      insight.action1 === "Generate Reviewer"
                                    ) {
                                      setGenerateModal(insight.topic);
                                      setGeneratePhase("loading");
                                      setTimeout(
                                        () => setGeneratePhase("preview"),
                                        2500,
                                      );
                                    } else {
                                ...
                                >
                                  {insight.action1 === "Generate Reviewer" && (
                                    <MaterialCommunityIcons
                                      name="robot"
                                      size={14}
                                      className={insight.btnText}
                                    />
                                  )}
                                  <Text
                                    className={`${insight.btnText} font-bold text-xs`}
                                    numberOfLines={1}
                                  >
                                    {insight.action1}
                                  </Text>
                                ...
                                <Pressable
                                  className={`flex-1 bg-white border border-${insight.color}-200 py-2 rounded-lg items-center active:bg-slate-50 shadow-sm shadow-${insight.color}-500/5`}
                                  onPress={() => alert(insight.action2)}
                                >
                                  <Text
                                    className={`text-${insight.color}-700 font-bold text-xs`}
                                    numberOfLines={1}
                                  >
                                    {insight.action2}
                                  </Text>
                  ...
                  {filter === "Resolved" &&
                    computedInsights.filter((ci) => ci.priority === "Resolved")
                      .length === 0 && (
          ...
          >
            ...
            >
              {classes.map((cls: any) => {
                ...
                return (
                  <View
                    key={cls.id}
                    className="bg-slate-50 p-4 rounded-xl border border-slate-200 mb-3 shadow-sm"
                  >
                    ...
                    <View className="flex-row justify-between items-center mb-2">
                      <Text className={`font-medium text-${color}-700 text-sm`}>
                        {label}
                      </Text>
                      <Text className={`font-black text-${color}-700`}>
                        {mainMetric}{" "}
                        <Text className="font-medium text-slate-400 text-xs">
                          / {data.total} students
                        </Text>
                      </Text>
                    </View>
            ...
            <Pressable
              onPress={() => setGraphModal(null)}
              className="w-full bg-slate-100 mt-4 py-3.5 rounded-xl items-center border border-slate-200 active:bg-slate-200 transition-colors"
            >
            ...
            {generatePhase === "loading" ? (
              <View className="items-center mb-6 mt-4">
                ...
                <Text className="text-slate-500 text-center text-sm leading-relaxed px-4">
                  ...
                  and drafting a localized 5-page summary with an adaptive
                  10-item quiz.
                </Text>
              </View>
            ) : (
              ...
              >
                <View className="bg-slate-50 p-4 rounded-xl border border-slate-200 mb-4 shadow-sm">
                  <Text className="font-bold text-slate-800 mb-2 text-base">
                    Module Summary: {generateModal}
                  </Text>
                  <Text className="text-slate-600 text-sm leading-relaxed">
                    This remediation module targets fundamental gaps in{" "}
                    {generateModal?.toLowerCase()}. It includes a simplified
                    reading guide and 5 multiple-choice questions to test
                    retention.
                  </Text>
                </View>
                <Text className="font-bold text-slate-700 mb-3 px-1 text-sm uppercase tracking-widest">
                  Generated Quiz Preview
                </Text>
                {[
                  {
                    type: "Multiple Choice",
                    q: "1. What is the main idea of the provided text?",
                    options: ["Option A", "Option B", "Option C", "Option D"],
                    ans: "Option B",
                  },
                  {
                    type: "Multiple Choice",
                    q: "2. Which of the following best describes the author's tone?",
                    options: [
                      "Informative",
                      "Persuasive",
                      "Argumentative",
                  ...
                  <View
                    key={i}
                    className="mb-4 bg-slate-50 p-4 rounded-xl border border-slate-200 shadow-sm"
                  >
                    <View className="flex-row justify-between mb-3">
                      <Text className="font-bold text-slate-800 flex-1 pr-2 text-sm">
                        {q.q}
                      </Text>
                      <Text className="text-[9px] font-bold text-rose-600 uppercase tracking-widest bg-rose-50 px-2 py-1 rounded-md self-start border border-rose-200">
                        {q.type}
                      </Text>
                    </View>
                    <View className="gap-2 mt-2">
                      {q.options.map((opt: string) => (
                        <View
                          key={opt}
                          className={`p-2.5 rounded-lg border ${q.ans === opt ? "bg-emerald-100 border-emerald-300 shadow-sm" : "bg-white border-slate-200"}`}
                        >
                          <Text
                            className={`text-xs ${q.ans === opt ? "text-emerald-900 font-black flex-row items-center gap-2" : "text-slate-600 font-medium"}`}
                          >
                            {opt} {q.ans === opt && "✓"}
                          </Text>
                        </View>
          ...
          >
            ...
            <View className="flex-row gap-3">
              ...
              <Pressable
                onPress={() => {
                  setNoticeModal(null);
                  alert("Notice broadcasted to class.");
                }}
                className="flex-[2] bg-blue-600 py-4.5 rounded-2xl items-center shadow-lg shadow-blue-500/30 active:bg-blue-700 transition-transform active:scale-95 flex-row justify-center gap-2"
              >
                <MaterialCommunityIcons name="send" size={18} color="white" />
                <Text className="text-white font-bold text-base">
                  Send to Class Dashboard
                </Text>
              </Pressable>
            </View>
          </GlobalModal>
