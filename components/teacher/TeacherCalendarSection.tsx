...
export const TeacherCalendarSection = () => {
  ...
  const ALL_EVENTS = [
    ...
    { id: 101, title: "12-HUMSS Chapter 1-3 Defense", date: "June 20, 2026", time: "10:00 AM", type: "Local Deadline", color: "orange", location: "Room 302", desc: "Final defense for chapter 1 to 3." },
    { id: 102, title: "11-STEM Literature Output", date: "June 25, 2026", time: "11:59 PM", type: "Local Deadline", color: "orange", location: "LMS Submission", desc: "Deadline for literature output." },
  ...
  const getEventsForDate = (y: number, m: number, d: number) => {
    ...
    if (y === 2026) {
      if (m === 5) { 
        ...
        if (d === 20) evs.push({ label: "Chapter 1-3 Defense", color: "orange" });
  ...
  return (
    ...
    >
      <ScrollView className="flex-1 w-full" contentContainerClassName="p-4 sm:p-6 flex-grow">
      <Animated.View entering={FadeIn} className="flex-1 w-full flex-col min-h-[600px] pb-10">
      <View className="mb-6 flex-col lg:flex-row lg:items-center justify-between gap-4">
        <View className="flex-row items-center gap-4">
          <View className="w-12 h-12 bg-indigo-50 rounded-xl items-center justify-center border border-indigo-100 shadow-sm">
            <MaterialCommunityIcons name="calendar-month" size={28} color="#4f46e5" />
          </View>
          <View>
      ...
      <View className="flex-col lg:flex-row gap-6 lg:gap-8 flex-1 pb-6">
            ...
            <View className="flex-row flex-wrap flex-1">
              {Array.from({ length: 35 }).map((_, i) => {
                ...
                return (
                  ...
                  >
                    <View className={`w-full h-full rounded-xl p-2 transition-all ${!isValidDate ? 'bg-transparent' : isToday ? 'bg-orange-500 shadow-md shadow-orange-500/30' : 'bg-slate-50 hover:bg-orange-50 active:bg-orange-100 border border-slate-100/50'}`}>
                      {isValidDate && (
                        <>
                          <View className="flex-row justify-between items-start mb-1">
                            <Text className={`text-xs sm:text-sm font-bold ${isToday ? "text-white" : "text-slate-700 group-hover:text-orange-900"}`}>
                            {date}
                          </Text>
                        </View>
                          ...
                          <View className="flex-col gap-1 overflow-hidden">
                            ...
                            {dayEvents.length > 3 && (
                              <Text className={`text-[9px] ${isToday ? 'text-orange-100' : 'text-slate-400'} font-bold ml-0.5 mt-0.5`}>+{dayEvents.length - 3} more</Text>
                            )}
                          </View>
                        </>
                      )}
        ...
        <View className="w-full lg:flex-[1] flex-col mb-6">
          ...
          >
            ...
            <Text className="text-white font-bold text-sm tracking-widest uppercase">Add Class Deadline</Text>
          </Pressable>
          ...
          <View className="flex-1 bg-white rounded-2xl p-4 sm:p-5 shadow-sm border border-slate-200/60 mt-4 flex-col">
             <View className="flex-row items-center justify-between mb-4 pb-4 border-b border-slate-100 z-10 relative">
               ...
               <View className="bg-orange-50 px-3 py-1 rounded-full border border-orange-100">
                  <Text className="text-orange-800 text-[10px] font-black uppercase tracking-widest">{ALL_EVENTS.length} Items</Text>
               </View>
             </View>
      ...
      >
        <ScrollView showsVerticalScrollIndicator={false} contentContainerClassName="pb-4">
          <View className="gap-5 mt-2 mb-8">
            ...
            <View className="flex-col sm:flex-row gap-5">
              ...
              <View className="flex-1">
                ...
                <View className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-4 flex-row items-center focus-within:border-orange-500 focus-within:bg-white shadow-sm transition-colors">
                  ...
                  <TextInput placeholder="e.g. Room 302" placeholderTextColor="#94a3b8" className="flex-1 outline-none text-slate-800 text-base" />
                </View>
              </View>
            </View>
            <View>
              <Text className="text-slate-700 font-bold mb-2 text-sm">Target Section</Text>
              <View className="flex-row flex-wrap gap-2">
                {["12-HUMSS-06", "11-STEM-02", "11-ABM-01", "All Classes"].map(scope => (
                  <Pressable key={scope} onPress={() => setNewEventScope(scope)} className={`px-4 py-2.5 rounded-xl border flex-row items-center gap-2 shadow-sm transition-colors ${newEventScope === scope ? 'bg-orange-50 border-orange-200' : 'bg-white border-slate-200'}`}>
                    <MaterialCommunityIcons name={newEventScope === scope ? "check-circle" : "circle-outline"} size={16} color={newEventScope === scope ? "#f97316" : "#94a3b8"} />
                    <Text className={`font-bold text-sm ${newEventScope === scope ? 'text-orange-700' : 'text-slate-600'}`}>{scope}</Text>
                  </Pressable>
                ))}
              </View>
            </View>
            <View className="flex-col sm:flex-row gap-5">
              <View className="flex-1">
                ...
                <View className="flex-row flex-wrap gap-2">
                  {["Local Deadline", "Assignment", "Assessment", "Enrichment"].map(type => (
          ...
          <View className="flex-row gap-3 w-full pb-2">
            ...
            >
              <MaterialCommunityIcons name="calendar-plus" size={20} color="white" />
              <Text className="text-white font-bold text-base">Deploy Event</Text>
            </Pressable>
          </View>
        </ScrollView>
      </GlobalModal>
