...
const PieChart = ({ data, size = 150, centerText }: { data: { percent: number, color: string, label?: string }[], size?: number, centerText?: string }) => {
  if (Platform.OS !== 'web') {
    return (
      <View style={{ width: size, height: size, borderRadius: size / 2, flexDirection: 'row', overflow: 'hidden', borderWidth: 6, borderColor: '#ffffff' }} className="shadow-md items-center justify-center relative">
        ...
        <View className="bg-white rounded-full shadow-sm absolute items-center justify-center" style={{ width: size * 0.75, height: size * 0.75 }}>
          {centerText && <Text className="font-black text-slate-800" style={{ fontSize: size * 0.18 }}>{centerText}</Text>}
        </View>
      </View>
    );
  }
  ...
  return (
    <View style={{ width: size, height: size, borderRadius: size / 2, backgroundImage: `conic-gradient(${gradientStops})` } as any} className="items-center justify-center shadow-md border-[6px] border-white relative">
      <View className="bg-white rounded-full shadow-sm absolute items-center justify-center" style={{ width: size * 0.75, height: size * 0.75 }}>
         {centerText && <Text className="font-black text-slate-800" style={{ fontSize: size * 0.18 }}>{centerText}</Text>}
      </View>
    </View>
...
export const TeacherClassAssessments = ({ classId }: { classId?: string }) => {
  ...
  const MOCK_ASSESSMENTS = classData?.assessments || [
    { id: 1, title: "Module 1: Basic Math", status: "Ongoing", type: "Mixed", date: today.toLocaleDateString('en-US'), items: 10, color: "rose", score: "--" },
    { id: 4, title: "Module 4: Reading Comprehension", status: "Ongoing", type: "Multiple Choice", date: today.toLocaleDateString('en-US'), items: 10, color: "rose", score: "--" },
    { id: 2, title: "Module 2: Primary Science", status: "Upcoming", type: "Multiple Choice", date: new Date(today.getTime() + 86400000 * 7).toLocaleDateString('en-US'), items: 10, color: "amber", score: "--" },
    { id: 5, title: "Module 5: Good Manners", status: "Upcoming", type: "Enumeration", date: new Date(today.getTime() + 86400000 * 14).toLocaleDateString('en-US'), items: 10, color: "amber", score: "--" },
    { id: 3, title: "Module 3: Shapes and Colors", status: "Completed", type: "Identification", date: new Date(today.getTime() - 86400000 * 10).toLocaleDateString('en-US'), items: 10, color: "emerald", score: "9/10" },
  ];
  ...
  }, [assessStatusFilter, assessTypeFilter, assessDateFilter, customStartDate, customEndDate, dateOffset]);
  ...
  return (
    <Animated.View entering={FadeIn} className="flex-1 flex-col pb-8 relative z-20">
      {activeDropdown && (
        <Pressable 
      ...
      <View className="flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-3 z-50 relative bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm">
        <View>
          <Text className="text-lg sm:text-xl font-black text-slate-900 tracking-tight">Class Assessments</Text>
          <Text className="text-xs sm:text-sm text-slate-500 font-medium mt-1">Manage formative modules designed for your students.</Text>
        </View>
      ...
      <View className="gap-3">
         ...
         {filteredAssessments.map((a: any) => (
            <Pressable onPress={() => handleAssessmentClick(a)} key={a.id} className={`bg-white p-3 sm:p-4 rounded-xl shadow-sm border flex-col sm:flex-row sm:items-center justify-between gap-3 transition-transform active:scale-[0.98] hover:shadow-md ${a.status === 'Ongoing' ? 'border-rose-300' : a.status === 'Upcoming' ? 'border-amber-200 opacity-80' : 'border-emerald-200'}`}>
              ...
              <View className="items-start sm:items-end flex-row sm:flex-col gap-2 w-full sm:w-auto mt-2 sm:mt-0 pt-2 sm:pt-0 border-t border-slate-100 sm:border-t-0">
                <View className={`px-2.5 py-1 rounded border ${a.status === 'Ongoing' ? 'bg-rose-100 border-rose-200' : a.status === 'Upcoming' ? 'bg-amber-100 border-amber-200' : 'bg-emerald-100 border-emerald-200'}`}>
                ...
                <Text className="text-slate-400 text-[10px] font-bold mt-1">
                  {a.status === 'Completed' ? 'Grading Done' : a.status === 'Ongoing' ? 'In Progress' : 'Scheduled'}
                </Text>
              </View>
            </Pressable>
         ))}
      ...
      <GlobalModal isOpen={customDateModal} onClose={() => setCustomDateModal(false)} title="Select Date Range">
         <View className="py-2 gap-4 w-full">
                  ...
                  {Array.from({ length: 42 }).map((_, i) => {
                    ...
                    return (
                      <Pressable key={i} disabled={!valid} onPress={() => valid && handleDayPress(dateNum)} className={`w-[14.28%] p-1 sm:p-1.5 h-10 sm:h-12 items-center justify-center`}>
                        {valid && (
                          <View className={`w-full h-full items-center justify-center rounded-lg ${isStartOrEnd ? 'bg-teal-600 shadow-md shadow-teal-500/30' : inRange ? 'bg-teal-100' : 'bg-transparent hover:bg-slate-100'}`}>
                            <Text className={`text-xs sm:text-sm font-bold ${isStartOrEnd ? 'text-white' : inRange ? 'text-teal-800' : 'text-slate-700'}`}>{dateNum}</Text>
                          </View>
                        )}
                      </Pressable>
                    );
            ...
            <View className="flex-row gap-3 mt-2 w-full">
              ...
              <Pressable onPress={() => { setCustomStartDate(tempStart); setCustomEndDate(tempEnd); setAssessDateFilter('Custom'); setCustomDateModal(false); }} disabled={!tempStart || !tempEnd} className={`flex-[2] py-4 rounded-xl items-center shadow-md active:scale-95 transition-transform ${tempStart && tempEnd ? 'bg-teal-600 hover:bg-teal-500 active:bg-teal-700 shadow-teal-500/30' : 'bg-teal-300'}`}>
      ...
      <GlobalModal isOpen={testPhase === 'preview'} onClose={() => { setTestPhase('none'); setActiveTest(null); }} title={`Preview: ${activeTest?.title}`}>
         {activeTest && (
            <ScrollView className="max-h-[60vh] py-2" showsVerticalScrollIndicator={false}>
               {activeTest?.questions?.map((q: any, i: number) => (
                  <View key={i} className="mb-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
                     <View className="flex-row justify-between mb-3">
                        <Text className="font-bold text-slate-800 flex-1 pr-2 text-base">{q.q}</Text>
                        <Text className="text-[10px] font-bold text-indigo-600 uppercase tracking-widest bg-indigo-50 px-2 py-1 rounded-md self-start border border-indigo-200">{q.type}</Text>
                     </View>
                     {q.type === 'Multiple Choice' && (
                        <View className="gap-2 mt-2">
                           {q.options?.map((opt: string) => (
                              <View key={opt} className={`p-3 rounded-lg border ${q.ans === opt ? 'bg-emerald-100 border-emerald-300 shadow-sm' : 'bg-white border-slate-200'}`}>
                                 <Text className={`text-sm ${q.ans === opt ? 'text-emerald-900 font-black flex-row items-center gap-2' : 'text-slate-600 font-medium'}`}>{opt} {q.ans === opt && "✓"}</Text>
                              </View>
                           ))}
                        </View>
                     )}
                     {q.type === 'Identification' && (
                        <View className="mt-2 p-3 bg-emerald-50 border border-emerald-200 rounded-lg shadow-sm">
                           <Text className="text-emerald-900 font-bold text-sm">Correct Answer: {q.ans}</Text>
                        </View>
                     )}
                     {q.type === 'Enumeration' && (
                        <View className="mt-2 p-3 bg-emerald-50 border border-emerald-200 rounded-lg shadow-sm">
                           <Text className="text-emerald-900 font-bold text-sm">Correct Answers: {Array.isArray(q.ans) ? q.ans.join(', ') : q.ans}</Text>
                        </View>
                     )}
                     
                     <View className="mt-4 pt-4 border-t border-slate-200">
                        <View className="flex-row items-center justify-between mb-2">
                          <Text className="text-xs font-bold text-slate-500 uppercase tracking-widest">Item Analysis (Correct Rate)</Text>
                          <Text className="text-xs font-black text-slate-800">{65 + ((i * 23) % 35)}%</Text>
                        </View>
                        <View className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                          <View className={`h-full rounded-full transition-all ${65 + ((i * 23) % 35) > 75 ? 'bg-emerald-500' : 'bg-amber-500'}`} style={{ width: `${65 + ((i * 23) % 35)}%` }} />
                        </View>
                     </View>
                  </View>
               ))}
               {(!activeTest?.questions || activeTest.questions.length === 0) && (
                 <Text className="text-slate-500 text-center py-8 font-medium">No questions currently defined for this module.</Text>
               )}
               <Pressable onPress={() => setTestPhase('results')} className="mt-2 w-full bg-slate-100 py-3.5 rounded-xl items-center shadow-sm active:bg-slate-200 transition-colors border border-slate-200">
                 <Text className="text-slate-700 font-bold text-sm">Back to Results Overview</Text>
               </Pressable>
            </ScrollView>
         )}
      </GlobalModal>

      <GlobalModal isOpen={testPhase === 'results'} onClose={() => { setTestPhase('none'); setActiveTest(null); }} title={`Results: ${activeTest?.title}`}>
         {activeTest && (() => {
           const board = getLeaderboard();
           const maxScore = activeTest.items || 10;
           const averageScore = board.length > 0 ? (board.reduce((acc: number, curr: any) => acc + curr.score, 0) / board.length).toFixed(1) : "0";
           const highestScore = board.length > 0 ? Math.max(...board.map((b: any) => b.score)) : 0;
           
           const passingThreshold = maxScore * 0.5;
           const passedCount = board.filter((b: any) => b.score >= passingThreshold).length;
           const passPercent = board.length > 0 ? Math.round((passedCount / board.length) * 100) : 0;
            ...
            <View className="py-2">
              <View className="flex-col sm:flex-row gap-4 mb-6">
                <View className="flex-[1] bg-white rounded-2xl p-4 shadow-sm border border-slate-200 flex-col items-center justify-center">
                   <Text className="text-sm font-black text-slate-800 mb-4 tracking-tight">Class Average</Text>
                   <PieChart centerText={`${passPercent}%`} data={[
                     { percent: passPercent, color: '#10b981', label: 'Passed' },
                     { percent: failPercent, color: '#f43f5e', label: 'Failed' }
                   ]} size={120} />
                   <View className="flex-row flex-wrap justify-center gap-x-4 gap-y-2 mt-4 w-full px-2">
                     <View className="flex-row items-center gap-1.5"><View className={`w-2.5 h-2.5 rounded-full bg-emerald-500`} /><Text className="text-slate-700 font-bold text-[10px] uppercase tracking-widest">Passed ({passPercent}%)</Text></View>
                     <View className="flex-row items-center gap-1.5"><View className="w-2.5 h-2.5 rounded-full bg-rose-500" /><Text className="text-slate-700 font-bold text-[10px] uppercase tracking-widest">Failed ({failPercent}%)</Text></View>
                   </View>
                </View>

                <View className="flex-[1.2] bg-white rounded-2xl p-4 shadow-sm border border-slate-200 flex-col justify-center">
                   <Text className="text-sm font-black text-slate-800 mb-3 tracking-tight">Key Metrics</Text>
                   <View className="gap-2">
                      <View className="bg-slate-50 border border-slate-100 p-2.5 rounded-xl flex-row justify-between items-center">
                         <Text className="text-slate-500 font-bold text-xs uppercase tracking-widest">Average Score</Text>
                         <Text className="text-slate-800 font-black text-base">{averageScore} / {maxScore}</Text>
                      </View>
                      <View className="bg-slate-50 border border-slate-100 p-2.5 rounded-xl flex-row justify-between items-center">
                         <Text className="text-slate-500 font-bold text-xs uppercase tracking-widest">Highest Score</Text>
                         <Text className="text-slate-800 font-black text-base">{highestScore} / {maxScore}</Text>
                      </View>
                      <View className="bg-slate-50 border border-slate-100 p-2.5 rounded-xl flex-row justify-between items-center">
                         <Text className="text-slate-500 font-bold text-xs uppercase tracking-widest">Completion Rate</Text>
                         <Text className="text-slate-800 font-black text-base">100%</Text>
                      </View>
                   </View>
                </View>
              </View>
              ...
              <View className="flex-row items-center justify-between mb-3 bg-slate-50 p-3 rounded-xl border border-slate-200">
                ...
                <View className="flex-row gap-1.5">
                  <Pressable onPress={() => setLbPage(Math.max(0, lbPage - 1))} className={`p-1 rounded-md border border-slate-200 bg-white active:bg-slate-100 shadow-sm ${lbPage === 0 ? 'opacity-40' : ''}`}>
                    <MaterialCommunityIcons name="chevron-left" size={16} color="#64748b" />
                  </Pressable>
                  <Pressable onPress={() => setLbPage(Math.min(Math.ceil(board.length / 5) - 1, lbPage + 1))} className={`p-1 rounded-md border border-slate-200 bg-white active:bg-slate-100 shadow-sm ${lbPage >= Math.ceil(board.length / 5) - 1 ? 'opacity-40' : ''}`}>
                    <MaterialCommunityIcons name="chevron-right" size={16} color="#64748b" />
                  </Pressable>
                </View>
              </View>
              <View className="gap-2 mb-4">
                 {board.slice(lbPage * 5, (lbPage + 1) * 5).map((lb: any, idx: number) => {
                    const realIdx = lbPage * 5 + idx;
                    return (
                      <View key={realIdx} className={`flex-row items-center justify-between p-3 rounded-xl border bg-slate-50 border-slate-200 shadow-sm hover:bg-slate-100 transition-colors`}>
                         <View className="flex-row items-center gap-3">
                            ...
                            <Text className={`font-bold text-sm text-slate-800`}>{lb.name}</Text>
                         </View>
                         <View className="items-end bg-white px-2 py-1 rounded-lg border border-slate-200 shadow-sm">
                           <Text className={`font-black text-sm text-slate-800`}>{lb.score} <Text className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">PTS</Text></Text>
                           <Text className="text-slate-500 font-bold text-[10px]">Max: {activeTest.items || 10}</Text>
                         </View>
                      </View>
                    );
                 })}
              ...
              <View className="flex-row gap-3">
                 <Pressable onPress={() => { setTestPhase('none'); setActiveTest(null); }} className="flex-1 bg-slate-100 py-3.5 rounded-xl border border-slate-200 items-center active:bg-slate-200 transition-colors">
                    <Text className="text-slate-700 font-bold text-sm">Close</Text>
                 </Pressable>
                 <Pressable onPress={() => setTestPhase('preview')} className="flex-1 bg-teal-600 py-3.5 rounded-xl items-center shadow-sm shadow-teal-500/30 active:bg-teal-700 transition-transform active:scale-95">
                    <Text className="text-white font-bold text-sm">Review Questions</Text>
                 </Pressable>
              </View>
            </View>
           </ScrollView>
