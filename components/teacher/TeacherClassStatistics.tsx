...
const TechDonut = ({
  ...
  return (
    <Pressable
      onPress={onPress}
      className="relative items-center justify-center active:scale-95 transition-transform"
    ...
    >
      ...
      <View
        className="absolute items-center justify-center bg-white/5 rounded-full flex-col shadow-inner"
        style={{
  ...
  if (Platform.OS !== "web") {
    return (
      ...
      >
        ...
        <View
          className="bg-white rounded-full shadow-sm absolute"
...
const STAT_CARDS = [
  {
    ...
    color: "blue",
    details:
      "The class average has improved significantly since the last formative assessment. 'Basic Math' is currently the strongest topic.",
  ...
  },
  {
    ...
    color: "emerald",
    details:
      "Overall attendance is excellent. However, 2 students have missed more than 3 consecutive online sessions.",
  ...
  },
  {
    ...
    color: "purple",
    details:
      "Peak AI usage occurs between 8 PM and 11 PM. Students are primarily querying about 'Reading Comprehension'.",
    action: "Open AI Chat Logs",
  },
  {
    ...
    color: "rose",
    details:
      "3 students are currently flagged by the predictive model due to declining scores and low platform engagement.",
    action: "Open Remediation Hub",
  },
];
...
const PREDICTIVE_INSIGHTS = [
  ...
  },
  {
    ...
    type: "success",
    details:
      "The new module 'Primary Science' has been accessed by 95% of the class.\nRecommendation: Continue using interactive modules.",
  },
];
...
export const TeacherClassStatistics = ({ classId }: { classId?: string }) => {
  ...
  const dynamicStats = useMemo(() => {
    return STAT_CARDS.map((stat) => {
      ...
      if (stat.value.includes("%")) {
        newValue =
          Math.min(100, Math.max(0, newNum)).toFixed(1).replace(".0", "") + "%";
  ...
  const dynamicChartData = useMemo(() => {
    ...
    return CHART_DATA.map((col) => ({
      ...col,
      val: Math.min(100, Math.max(10, col.val + col.val * dateOffset * 0.15)),
    }));
  }, [dateOffset]);
  ...
  return (
    <Animated.View entering={FadeIn} className="w-full flex-col min-h-[500px]">
      ...
      <View className="flex-col sm:flex-row justify-between sm:items-center mb-4 gap-3 border-b border-slate-200 pb-3 bg-white p-4 sm:p-5 rounded-2xl shadow-sm z-10">
        <View className="flex-1">
          ...
          <Text className="text-slate-600 text-xs sm:text-sm mt-1 leading-relaxed font-medium">
            Predictive insights, class performance, and automated student
            tracking.
          </Text>
        </View>
        <Pressable
      ...
      <View className="px-2 pb-8 pt-2 gap-4">
          ...
          <View className="flex-row items-center gap-2 relative z-[60]">
            <View className="relative">
              <Pressable
                ...
                className="bg-white border border-slate-200 px-3 py-2 rounded-lg flex-row items-center gap-2 shadow-sm min-w-[100px] justify-between active:bg-slate-50 transition-colors"
              >
              ...
              {activeDropdown === "date" && (
                <View
                  className="absolute top-full mt-1 left-0 w-36 bg-white border border-slate-200 rounded-lg shadow-xl overflow-hidden"
        ...
        <View className="-mx-2 px-2 z-20">
          <ScrollView
            ...
            contentContainerClassName="flex-row gap-4 pr-4"
          >
            {dynamicStats.map((stat, idx) => (
              <Animated.View
                key={idx}
                entering={FadeInDown.delay(50 * idx)}
                className="w-[200px] sm:w-[240px] flex-none mb-2"
              >
                <Pressable
                  onPress={() => setSelectedStat(stat)}
                  className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm flex-col relative overflow-hidden h-full"
                >
                  <View className="flex-row items-start justify-between mb-4">
                    <View
                      className={`w-10 h-10 rounded-xl items-center justify-center border shadow-sm ${getColorStyles(stat.color)}`}
                    >
                      <MaterialCommunityIcons
                  ...
                  <Text className="text-slate-600 font-bold text-sm mb-1">
                    {stat.title}
                  </Text>
                  ...
                  <Text
                    className={`text-xs font-bold mt-2 flex-row items-center ${stat.trendUp ? "text-emerald-600" : "text-rose-600"}`}
                  >
                    <MaterialCommunityIcons
            ...
            >
              ...
              <View className="flex-row justify-between items-center mb-4 pr-10">
                <View>
                  ...
                  <Text className="text-slate-500 text-sm mt-1">
                    Aggregated platform interactions.
                  </Text>
                </View>
              </View>
              ...
              <View className="h-56 flex-row items-end justify-between gap-2 sm:gap-4 mt-4">
                {dynamicChartData.map((col, i) => (
                  <View
                    key={i}
                    className="flex-1 items-center gap-2 group/bar h-full justify-end"
                  >
            ...
            >
              ...
              <View className="flex-row flex-wrap justify-around mt-4 gap-y-6">
                {[
                  ...
                  <View
                    key={i}
                    className="items-center bg-slate-800 rounded-3xl p-3 w-[45%]"
                  >
                    <TechDonut
                      size={70}
                      strokeWidth={8}
                      percent={topic.val}
                      color={topic.color}
                      trackColor={topic.track}
                    />
                    <Text className="font-bold text-white text-xs mt-2">
                      {topic.label}
                    </Text>
                  </View>
                ))}
        ...
        <Animated.View entering={FadeInDown.delay(600)} className="mt-6 mb-2">
          <View className="flex-row justify-between items-center px-2 mb-4">
            ...
            <View className="bg-emerald-100 px-3 py-1 rounded-full">
              <Text className="text-emerald-800 text-[10px] font-black uppercase tracking-widest">
                Top 5 Students
              </Text>
            </View>
          </View>
          <View className="bg-white border border-slate-200 rounded-2xl shadow-sm p-4">
            {CLASS_LEADERBOARD.map((student, idx) => (
              <View
                key={idx}
                className="flex-row items-center justify-between p-3 border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors"
              >
                <View className="flex-row items-center gap-3">
                  <View
                    className={`w-8 h-8 rounded-full items-center justify-center shadow-sm ${idx === 0 ? "bg-amber-100 border border-amber-200" : idx === 1 ? "bg-slate-200 border border-slate-300" : idx === 2 ? "bg-orange-100 border border-orange-200" : "bg-white border border-slate-200"}`}
                  >
                    {idx === 0 ? (
        ...
        <Animated.View entering={FadeInDown.delay(500)} className="gap-4 mt-2">
          <View className="flex-row justify-between items-center px-2 mb-2">
            ...
            <View className="bg-indigo-100 px-3 py-1 rounded-full">
              <Text className="text-indigo-800 text-[10px] font-black uppercase tracking-widest">
                {PREDICTIVE_INSIGHTS.length} New
              </Text>
            </View>
          </View>
          ...
          <View className="flex-col lg:flex-row gap-4 flex-wrap w-full">
            {PREDICTIVE_INSIGHTS.map((insight) => (
              ...
              >
                ...
                <View
                  className={`py-3 rounded-lg items-center shadow-sm mt-auto ${
                    insight.type === "danger"
      ...
      >
        <View className="mb-6 mt-2 gap-4">
          <Text className="text-slate-600 text-sm mb-2">
            Select the data range and format to export the class statistics
            report.
          </Text>
          <View>
        ...
        <View className="flex-row gap-3">
          ...
          <Pressable
            ...
            className="flex-1 bg-slate-900 py-4 rounded-xl items-center shadow-md active:bg-slate-800 active:scale-95 transition-all"
          >
      ...
      >
        {selectedInsight && (
          <View className="mt-2 mb-6">
            <View
              className={`w-20 h-20 rounded-full items-center justify-center mb-6 border-4 self-center shadow-sm ${
            ...
            <Text className="text-2xl font-black text-slate-800 text-center mb-2 px-4 tracking-tight">
              {selectedInsight.title}
            </Text>
            <Text className="text-slate-600 text-center text-sm leading-relaxed px-4 mb-6">
            ...
            <View className="flex-row gap-3">
              ...
              <Pressable
                ...
                className={`flex-[2] py-4 rounded-xl items-center shadow-md active:scale-[0.98] transition-transform ${
                  selectedInsight.type === "danger"
      ...
      <GlobalModal
        ...
        title={`${selectedStat?.title} Overview`}
      >
        {selectedStat && (
          <View className="mt-2 mb-6">
            <View
              className={`w-24 h-24 rounded-full items-center justify-center mb-6 border-4 self-center shadow-sm ${
                selectedStat.color === "blue"
                  ? "bg-blue-100 border-blue-50"
                  : selectedStat.color === "emerald"
                    ? "bg-emerald-100 border-emerald-50"
                    : selectedStat.color === "purple"
            ...
            <View className="items-center mb-6">
              ...
              <View
                className={`flex-row items-center gap-1 mt-2 px-3 py-1 rounded-full ${selectedStat.trendUp ? "bg-emerald-50" : "bg-rose-50"}`}
              >
                <MaterialCommunityIcons
                  name={selectedStat.trendUp ? "trending-up" : "trending-down"}
                  size={16}
                  color={selectedStat.trendUp ? "#059669" : "#e11d48"}
                />
                <Text
                  className={`font-bold text-sm ${selectedStat.trendUp ? "text-emerald-700" : "text-rose-700"}`}
                >
                  {selectedStat.trend}
                </Text>
              </View>
            ...
            <View className="flex-row gap-3">
              ...
              <Pressable
                onPress={() => {
                  ...
                  alert(`Navigating to ${selectedStat.action}...`);
                }}
                className={`flex-[2] py-4 rounded-xl items-center shadow-md active:scale-[0.98] transition-transform ${
                  selectedStat.color === "blue"
      ...
      <GlobalModal
        ...
        title="System Engagement Analysis"
      >
        <View className="mb-6 mt-2">
          <Text className="text-slate-600 mb-6 leading-relaxed">
            A detailed breakdown of how students are interacting with the LMS
            and AI features this week. High engagement correlates with improved
            mastery.
          </Text>
          ...
          <View className="flex-row gap-4 mb-6">
            <View className="flex-1 bg-blue-50 p-5 rounded-2xl border border-blue-100 items-center shadow-sm">
              ...
              <Text className="text-[10px] font-bold text-blue-600 uppercase tracking-widest text-center mt-1">
                Total Interactions
              </Text>
            </View>
            <View className="flex-1 bg-purple-50 p-5 rounded-2xl border border-purple-100 items-center shadow-sm">
              ...
              <Text className="text-[10px] font-bold text-purple-600 uppercase tracking-widest text-center mt-1">
                Avg Session
              </Text>
            </View>
          </View>

          <Text className="font-bold text-slate-800 mb-3 text-lg border-b border-slate-100 pb-2">
            Most Active Modules
          </Text>
          <View className="gap-3">
            <View className="bg-slate-50 border border-slate-200 p-4 rounded-xl flex-row justify-between items-center hover:bg-white transition-colors">
              <Text className="text-slate-800 font-bold">
                1. Primary Science Practice Test
              </Text>
              <Text className="text-slate-500 font-black text-sm bg-white px-3 py-1 rounded-lg border border-slate-200">
                1.2k views
              </Text>
            </View>
            <View className="bg-slate-50 border border-slate-200 p-4 rounded-xl flex-row justify-between items-center hover:bg-white transition-colors">
        ...
        <Pressable
          onPress={() => setEngagementModalOpen(false)}
          className="w-full bg-slate-900 py-4 rounded-xl items-center shadow-md active:bg-slate-800 active:scale-[0.98] transition-all"
        >
      ...
      >
        ...
        >
          <View className="mb-6 mt-2 pb-2">
            <Text className="text-slate-600 mb-6 leading-relaxed">
              Competency is calculated by cross-referencing continuous
              assessment scores with AI chat evaluation confidence markers.
            </Text>
              ...
              {[
                {
                  ...
                  status: "Mastered",
                  color: "emerald",
                  desc: "Most students demonstrate a strong conceptual understanding.",
                },
                {
                  ...
                  status: "Proficient",
                  color: "blue",
                  desc: "Solid overall performance, minor issues with advanced topics.",
                },
                {
                  ...
                  status: "Developing",
                  color: "amber",
                  desc: "Class is currently learning this topic. Scores are stabilizing.",
                },
                {
                  ...
                  status: "At-Risk",
                  color: "rose",
                  desc: "Significant confusion detected in RAG queries. Remediation recommended.",
                },
              ].map((topic, i) => (
                <View
                  key={i}
                  className={`p-5 rounded-2xl border border-${topic.color}-200 bg-${topic.color}-50 shadow-sm`}
                >
                  <View className="flex-row justify-between items-center mb-2">
                    <Text
                      className={`font-black text-${topic.color}-900 text-lg`}
                    >
                      {topic.label}
                    </Text>
                    <View
                      className={`bg-${topic.color}-100 px-2.5 py-1 rounded-lg border border-${topic.color}-200/50`}
                    >
                      <Text
                        className={`text-[10px] font-black text-${topic.color}-800 uppercase tracking-widest`}
                      >
                        {topic.status}
                      </Text>
                    </View>
                  ...
                  <Text
                    className={`text-${topic.color}-800 text-sm font-medium mb-4 leading-relaxed`}
                  >
                    {topic.desc}
                  </Text>
                  <View className="flex-row items-center gap-4">
                    ...
                    <Text
                      className={`font-black text-${topic.color}-900 text-base`}
                    >
                      {topic.val}%
                    </Text>
                  </View>
        ...
        <Pressable
          onPress={() => setMasteryModalOpen(false)}
          className="w-full bg-slate-900 py-4 rounded-xl items-center shadow-md active:bg-slate-800 active:scale-[0.98] transition-all"
        >
        ...
        <View className="py-2 gap-4 w-full">
          ...
          <View className="flex-row gap-3 mt-2 w-full">
            <Pressable
              ...
              className="flex-1 bg-slate-100 py-3 rounded-lg items-center border border-slate-200 hover:bg-slate-200 active:bg-slate-300 transition-colors"
            >
            ...
            <Pressable
              ...
              className={`flex-[2] py-3 rounded-lg items-center shadow-md active:scale-95 transition-transform ${tempStart && tempEnd ? "bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 shadow-indigo-500/30" : "bg-indigo-300"}`}
            >
