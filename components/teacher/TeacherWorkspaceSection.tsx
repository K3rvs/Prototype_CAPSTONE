...
import { Pressable, ScrollView, Text, TextInput, View, Platform, TouchableOpacity } from "react-native";
...
export const TeacherWorkspaceSection = ({ setActiveTab }: any) => {
  ...
  const METRICS = [
    { id: 1, title: "Total Students", value: "123", trend: "+2 this month", trendUp: true, icon: "account-group", color: "text-blue-600", bg: "bg-blue-50", graphColor: "bg-blue-500", data: [30, 40, 45, 50, 48, 60, 70] },
    { id: 2, title: "Avg Attendance", value: "96%", trend: "Stable", trendUp: true, icon: "calendar-check", color: "text-teal-600", bg: "bg-teal-50", graphColor: "bg-teal-500", data: [90, 95, 92, 98, 96, 99, 98] },
    { id: 3, title: "SBTs Awarded", value: "45", trend: "+12 this week", trendUp: true, icon: "medal-outline", color: "text-purple-600", bg: "bg-purple-50", graphColor: "bg-purple-500", data: [20, 35, 30, 55, 50, 75, 80] },

    { id: 5, title: "AI Queries", value: "842", trend: "Peak Usage", trendUp: true, icon: "robot-outline", color: "text-rose-600", bg: "bg-rose-50", graphColor: "bg-rose-500", data: [60, 80, 75, 90, 85, 95, 100] },
  ];

  const ACTION_ITEMS = [
    { id: "a1", user: "12-HUMSS-06", type: "Action", text: "Grade PR2 Midterms - Pending submissions from 42 students.", time: "Due Today", icon: "check-decagram", color: "text-blue-600", bg: "bg-blue-100", details: "Grades are due to the principal by Friday.", actionLabel: "Grade Now" },
    { id: "a2", user: "11-STEM-02", type: "Alert", text: "Review GenMath Logs - High Confusion on Rational Functions.", time: "High Alert", icon: "alert-octagon", color: "text-rose-600", bg: "bg-rose-100", details: "AI Tutor flagged multiple students struggling.", actionLabel: "Review Logs" },
    { id: "a3", user: "11-ABM-01", type: "Action", text: "Approve 3 Students - New join requests pending in your section.", time: "Pending", icon: "account-clock", color: "text-amber-600", bg: "bg-amber-100", details: "Please review the roster.", actionLabel: "View Roster" },
    { id: "a4", user: "System", type: "System", text: "Syllabus Sync Complete - Vector DB updated.", time: "1 hour ago", icon: "database-sync-outline", color: "text-emerald-600", bg: "bg-emerald-100", details: "RAG Context successfully rebuilt for all classes.", actionLabel: "View Details" },
  ];

  const FAST_ACTIONS = [
    { id: "classrooms", label: "My Classes", desc: "Manage classes & materials", icon: "google-classroom", iconBg: "bg-teal-100", iconColor: "#0f766e", textColor: "text-teal-800", hoverBorder: "hover:border-teal-300", hoverShadow: "hover:shadow-teal-500/20", blob: "bg-teal-400" },
    { id: "calendar", label: "Calendar", desc: "School-wide events", icon: "calendar-month", iconBg: "bg-blue-100", iconColor: "#1d4ed8", textColor: "text-blue-800", hoverBorder: "hover:border-blue-300", hoverShadow: "hover:shadow-blue-500/20", blob: "bg-blue-400" },
    { id: "remediation", label: "Remediation", desc: "At-Risk triage", icon: "medical-bag", iconBg: "bg-rose-100", iconColor: "#e11d48", textColor: "text-rose-800", hoverBorder: "hover:border-rose-300", hoverShadow: "hover:shadow-rose-500/20", blob: "bg-rose-400" },
    { id: "faq", label: "Help & FAQ", desc: "Support & guides", icon: "help-circle-outline", iconBg: "bg-indigo-100", iconColor: "#4f46e5", textColor: "text-indigo-800", hoverBorder: "hover:border-indigo-300", hoverShadow: "hover:shadow-indigo-500/20", blob: "bg-indigo-400" },
  ];
  ...
  return (
    ...
    >
      ...
      {/* 1. HERO WIDGET */}
      <Animated.View entering={FadeInDown.delay(100)} className="w-full bg-gradient-to-br from-slate-900 via-teal-950 to-slate-900 rounded-3xl p-6 sm:p-8 shadow-2xl relative overflow-hidden border border-slate-800">
        ...
        <View className="flex-col sm:flex-row sm:items-center justify-between gap-6 z-10">
           <View className="flex-1">
             ...
             <Text className="text-3xl sm:text-4xl font-black text-white mb-3 tracking-tight leading-tight">Good morning, Mr. Santos!</Text>
             <Text className="text-slate-300 text-sm sm:text-base leading-relaxed max-w-2xl font-medium mb-6 sm:mb-0">Your AI Assistant handled <Text className="text-white font-bold">42 queries</Text> overnight. You have <Text className="text-rose-400 font-bold">3 critical alerts</Text> requiring attention today.</Text>
           </View>
           <Pressable onPress={() => setQuickPostModal(true)} className="bg-teal-600 hover:bg-teal-500 px-6 py-3.5 rounded-2xl flex-row items-center justify-center gap-2 shadow-xl shadow-teal-900/30 border border-teal-500 transition-colors sm:self-center">
              <MaterialCommunityIcons name="bullhorn" size={24} color="white" />
              <Text className="text-white font-black text-sm uppercase tracking-widest">Quick Broadcast</Text>
           </Pressable>
        </View>
      </Animated.View>
      ...
      <View className="flex-col xl:flex-row gap-4 sm:gap-6 mb-6 px-2 w-full">
        ...
        <View className="flex-1 flex-col w-full h-full">
          <View className="flex-row items-center justify-between mb-3 px-1">
            ...
            <TouchableOpacity activeOpacity={0.7} onPress={() => setActiveTab("classrooms")} className="flex-row items-center gap-1 bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-full transition-colors border border-slate-200">
      ...
      {/* Details Modal */}
      <GlobalModal isOpen={!!selectedActivity} onClose={() => setSelectedActivity(null)} title="Action Details">
        {selectedActivity && (
          <View className="py-2">
            <View className="flex-row items-center gap-4 mb-4">
              ...
              <View className="flex-1">
                <Text className="text-lg font-black text-slate-900 tracking-tight">{selectedActivity.title}</Text>
                <Text className="text-slate-500 font-medium text-xs mt-0.5">{selectedActivity.time} • {selectedActivity.user}</Text>
              </View>
            ...
            <View className="flex-row gap-3">
              ...
              <Pressable onPress={() => { setSelectedActivity(null); setActiveTab("classrooms"); }} className={`flex-1 py-3 rounded-lg items-center shadow-md transition-transform active:scale-95 bg-teal-600 active:bg-teal-700 shadow-teal-500/20`}>
                <Text className="text-white font-bold text-sm">Review Now</Text>
              </Pressable>
            </View>
          </View>
        )}
