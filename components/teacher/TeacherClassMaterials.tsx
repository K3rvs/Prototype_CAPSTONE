...
const MOCK_MATERIALS = [
  { id: 1, name: "PR2_Course_Syllabus.pdf", type: "pdf", size: "2.4 MB", date: new Date(today.getTime() - 86400000 * 20).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }), published: true },
  { id: 2, name: "Chapter_1_Introduction.pptx", type: "presentation", size: "14.1 MB", date: new Date(today.getTime() - 86400000 * 15).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }), published: true },
  { id: 3, name: "Reading_Assignment_1.docx", type: "document", size: "856 KB", date: new Date(today.getTime() - 86400000 * 10).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }), published: false },
  { id: 4, name: "Midterm_Reviewer_Answers.pdf", type: "pdf", size: "1.2 MB", date: new Date(today.getTime() - 86400000 * 5).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }), published: false },
];
...
export const TeacherClassMaterials = ({ classId }: { classId?: string }) => {
  ...
  const [materials, setMaterials] = useState(classData?.materials || MOCK_MATERIALS as any);
  ...
  const togglePublish = (id: number) => {
    const updated = materials.map((m: any) => m.id === id ? { ...m, published: !m.published } : m);
      ...
      <View className="flex-1 bg-slate-50/50 flex-col">
        {subTab === "materials" && (
          <Animated.View entering={FadeIn} className="flex-1 w-full bg-white">
            <ScrollView showsVerticalScrollIndicator={true} className="flex-1 w-full" contentContainerClassName="flex-grow p-4 sm:p-6 flex-col max-w-5xl mx-auto w-full gap-3 pb-12">
      {/* Header */}
              <View className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm z-10 flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                <View className="flex-1">
                  <Text className="text-lg sm:text-xl font-black text-slate-800 mb-1 tracking-tight">Class Materials</Text>
                  <Text className="text-slate-500 text-xs sm:text-sm font-medium">Resources uploaded for students to read. These also serve as the knowledge base for the AI Tutor.</Text>
                </View>
                <Pressable
                ...
                >
                  ...
                  <Text className="text-white font-bold uppercase tracking-widest text-[10px]">Upload Material</Text>
                </Pressable>
              </View>
        ...
        {materials.map((file: any, idx: number) => {
          ...
          return (
            <Animated.View key={file.id} entering={FadeInDown.delay(50 * idx)} className="w-full bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-slate-200 flex-row items-center justify-between gap-3 hover:shadow-md transition-shadow">
              <View className="flex-row items-center gap-3 flex-1">
                <View className={`w-10 h-10 rounded-lg items-center justify-center border border-white shadow-sm ${ui.bg}`}>
                  <MaterialCommunityIcons name={ui.icon as any} size={20} color={ui.color} />
                </View>
                <View className="flex-1">
                  <Text className="font-bold text-slate-800 text-sm leading-tight">{file.title}</Text>
                  <Text className="text-slate-500 text-[10px] sm:text-xs font-medium uppercase tracking-wider">{file.type} • {file.size}</Text>
                </View>
              </View>
              
              <View className="flex-row items-center gap-2">
                <Pressable onPress={() => togglePublish(file.id)} className={`px-2.5 py-1.5 rounded border flex-row items-center gap-1 transition-colors ${file.published ? "bg-emerald-50 border-emerald-200" : "bg-slate-100 border-slate-300"}`}>
                  <MaterialCommunityIcons name={file.published ? "eye-outline" : "eye-off-outline"} size={14} color={file.published ? "#059669" : "#64748b"} />
                  <Text className={`text-[10px] font-bold uppercase tracking-widest ${file.published ? "text-emerald-700" : "text-slate-600"}`}>{file.published ? "Pub'd" : "Hid"}</Text>
                </Pressable>
                ...
                <View className="flex-row gap-1 border-l border-slate-200 pl-2">
                  ...
                  <Pressable onPress={() => {
                      const updated = materials.filter((m: any) => m.id !== file.id);
                      setMaterials(updated);
                      if (classData) classData.materials = updated;
                  }} className="w-8 h-8 rounded items-center justify-center hover:bg-red-50 active:bg-red-100 transition-colors group/del">
                    <MaterialCommunityIcons name="trash-can-outline" size={16} color="#64748b" className="group-hover/del:text-red-500" />
                  </Pressable>
                </View>
              </View>
            </Animated.View>
        ...
        {subTab === "context" && (
          <Animated.View entering={FadeIn} className="flex-1 w-full bg-white">
            ...
            <View className="flex-col sm:flex-row flex-wrap gap-4 w-full">
               <Pressable onPress={() => setContextUploadModal(true)} className="w-full sm:flex-1 bg-white border border-slate-200 p-4 rounded-xl shadow-sm hover:border-teal-400 hover:shadow-md transition-all group items-start active:scale-[0.98]">
                  <View className="w-10 h-10 bg-teal-50 rounded-lg items-center justify-center mb-2 group-hover:bg-teal-500 transition-colors border border-teal-100 group-hover:border-teal-600">
                     <MaterialCommunityIcons name="file-upload-outline" size={20} color="#0f766e" className="group-hover:text-white" />
                  </View>
               ...
               <Pressable onPress={() => setDriveSyncModal(true)} className="w-full sm:flex-1 bg-white border border-slate-200 p-4 rounded-xl shadow-sm hover:border-blue-400 hover:shadow-md transition-all group items-start active:scale-[0.98]">
                  <View className="w-10 h-10 bg-blue-50 rounded-lg items-center justify-center mb-2 group-hover:bg-blue-500 transition-colors border border-blue-100 group-hover:border-blue-600">
                     <MaterialCommunityIcons name="google-drive" size={20} color="#1d4ed8" className="group-hover:text-white" />
                  </View>
               ...
               <Pressable onPress={() => setUrlSyncModal(true)} className="w-full sm:flex-1 bg-white border border-slate-200 p-4 rounded-xl shadow-sm hover:border-purple-400 hover:shadow-md transition-all group items-start active:scale-[0.98]">
                  <View className="w-10 h-10 bg-purple-50 rounded-lg items-center justify-center mb-2 group-hover:bg-purple-500 transition-colors border border-purple-100 group-hover:border-purple-600">
                     <MaterialCommunityIcons name="link-variant" size={20} color="#7e22ce" className="group-hover:text-white" />
                  </View>
            ...
            <View className="flex-col sm:flex-row items-start sm:items-center justify-between mt-4 mb-2 px-2 gap-4">
              <Text className="font-black text-slate-800 text-base">Indexed Materials</Text>
            ...
            <View className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-2 mb-10 overflow-hidden shadow-inner h-[300px]">
              <ScrollView showsVerticalScrollIndicator={true} nestedScrollEnabled={true} className="flex-1 w-full" contentContainerClassName="gap-3 pb-4">
               {filteredIndexedFiles.map((file, idx) => (
                   <Animated.View entering={FadeInDown.delay(50 * idx)} key={file.id} className="bg-white border border-slate-200 p-3 sm:p-4 rounded-xl flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm mx-1 mt-1">
                   <View className="flex-row items-center gap-4 flex-1">
                     <View className={`w-10 h-10 rounded-lg items-center justify-center shadow-sm flex-shrink-0 ${file.type === 'pdf' ? 'bg-red-50 border border-red-100' : file.type === 'ppt' ? 'bg-orange-50 border border-orange-100' : 'bg-blue-50 border border-blue-100'}`}>
                       <MaterialCommunityIcons name={file.type === 'pdf' ? 'file-pdf-box' : file.type === 'ppt' ? 'file-powerpoint-box' : 'link-variant'} size={20} color={file.type === 'pdf' ? '#ef4444' : file.type === 'ppt' ? '#f97316' : '#3b82f6'} />
                     </View>
                     <View className="flex-1">
                       <Text className="text-slate-900 font-bold text-sm mb-1 group-hover:text-teal-700 transition-colors" numberOfLines={1}>{file.name}</Text>
