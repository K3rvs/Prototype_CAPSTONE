          ...
          <View className="flex-row items-center gap-4 relative z-10">
            ...
            <View>
              ...
              <Text className="text-slate-500 text-sm font-medium mt-0.5">
                Review NLP AI flagged incidents in real-time
              </Text>
            </View>
          </View>
        </View>
        ...
        <View className="p-3 sm:p-4 flex-1 bg-slate-50/50">
          <Animated.View entering={FadeIn} className="flex-1 flex-col">
            ...
            <View className="flex-col lg:flex-row gap-6 flex-1">
              {/* Incident List */}
              <View
                className={`w-full lg:w-[320px] xl:w-[380px] border-b lg:border-b-0 lg:border-r border-slate-200 pb-6 lg:pr-6 flex-col gap-3 flex-1 ${selectedIncident ? "hidden lg:flex" : "flex"}`}
              >
                <ScrollView
                    ...
                    filteredIncidents.map((incident, idx) => {
                      ...
                      return (
                        <Pressable
                          ...
                          className={`p-3 rounded-xl border-2 mb-2 flex-row gap-3 ${isSelected ? `${styles.bg} ${styles.border}` : "bg-white border-slate-100 shadow-sm"}`}
                        >
                          <View
                            className={`w-6 h-6 rounded-full items-center justify-center flex-shrink-0 ${isSelected ? "bg-white/60" : styles.bg}`}
                          >
                            <Text
              ...
              {/* Incident Details */}
              <View
                className={`flex-1 flex-col overflow-hidden ${!selectedIncident ? "hidden lg:flex" : "flex"}`}
              >
                {selectedIncident ? (
                  <Animated.View
                    entering={FadeIn.duration(300)}
                    className={`border-l-4 ${getPriorityStyles(selectedIncident.priority, selectedIncident.status).border} ${getPriorityStyles(selectedIncident.priority, selectedIncident.status).bg} p-4 sm:p-5 rounded-xl shadow-sm flex-1 flex-col justify-between overflow-hidden`}
                  >
                    <ScrollView
                      className="flex-1"
                    ...
                    >
                      <View>
                        <Pressable
                          ...
                          className="lg:hidden flex-row items-center gap-2 mb-4 p-2 bg-white rounded-lg self-start border border-slate-200 shadow-sm active:bg-slate-50"
                        >
                          <MaterialCommunityIcons
                            name="arrow-left"
                            size={20}
                        ...
                        <View className="flex-col sm:flex-row sm:justify-between sm:items-start mb-4 gap-2">
                          <Text
                            className={`font-black text-xl sm:text-2xl ${getPriorityStyles(selectedIncident.priority, selectedIncident.status).text}`}
                          >
                        ...
                        <Text
                          className={`text-sm mb-4 font-medium ${getPriorityStyles(selectedIncident.priority, selectedIncident.status).text}`}
                        >
                    ...
                    {selectedIncident.status !== "Actioned" ? (
                      <View className="flex-col sm:flex-row gap-3">
                        <Pressable
                          ...
                          className="flex-1 bg-white py-3 rounded-lg items-center justify-center border border-slate-200 shadow-sm active:bg-slate-50 transition-transform"
                        >
                        ...
                        <Pressable
                          ...
                          className="flex-1 bg-orange-600 py-3 rounded-lg items-center justify-center border border-orange-700 shadow-lg shadow-orange-500/20 active:bg-orange-700 transition-transform"
                        >
            ...
            >
              ...
              <View className="flex-row gap-3">
                ...
                <Pressable
                  ...
                  className="flex-1 bg-orange-600 py-3 rounded-lg items-center shadow-md shadow-orange-500/30 active:bg-orange-700 transition-colors"
                >
