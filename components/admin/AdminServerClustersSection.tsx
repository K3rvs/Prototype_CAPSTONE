            ...
            >
              ...
              <View className="flex-row flex-wrap gap-4">
                <View className="flex-1 min-w-[200px] bg-slate-900 rounded-2xl p-5 shadow-xl relative overflow-hidden border border-slate-800">
                  ...
                  <Text className="text-3xl font-black text-white tracking-tight mb-1 z-10">
                    99.98%
                  </Text>
                  <Text className="text-slate-400 text-xs font-bold uppercase tracking-widest z-10">
                    Global Uptime
                  </Text>
                </View>
                ...
                <View className="flex-1 min-w-[200px] bg-white rounded-2xl p-5 shadow-sm border border-slate-200 flex-col justify-between group hover:border-blue-300 transition-colors">
                  ...
                  <Text className="text-3xl font-black text-slate-800 tracking-tight mb-1">
                    51%
                  </Text>
                ...
                <View className="flex-row flex-wrap gap-4">
                  {CLUSTERS.map((c, idx) => (
                    <Animated.View
                      ...
                      className="w-full xl:w-[calc(50%-8px)]"
                    >
                      <Pressable
                        onPress={() => setManageNode(c)}
                        className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm flex-col gap-4 hover:border-blue-300 hover:shadow-md transition-all active:scale-[0.99] group"
                      >
                        {/* Card Header */}
                        <View className="flex-row items-center justify-between">
                          <View className="flex-row items-center gap-3">
                            <View
                              className={`w-12 h-12 rounded-xl items-center justify-center border shadow-sm ${c.status === "Healthy" ? "bg-emerald-50 border-emerald-200" : "bg-amber-50 border-amber-200"}`}
                            >
                              <MaterialCommunityIcons
                                name={
                                  c.status === "Healthy"
                                ...
                                size={24}
                                color={
                                  c.status === "Healthy" ? "#059669" : "#d97706"
                                }
                              />
                            </View>
                            <View>
                          ...
                          <View
                            className={`px-2.5 py-1 rounded-md border shadow-sm ${c.status === "Healthy" ? "bg-emerald-50 border-emerald-200" : "bg-amber-50 border-amber-200"}`}
                          >
                            <Text
                        ...
                        <View className="flex-col gap-3 mt-2">
                          <View className="flex-col gap-1">
                            ...
                            <View className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                              <View
                                className={`h-full rounded-full transition-all duration-500 ${c.cpu > 80 ? "bg-amber-500" : "bg-blue-500"}`}
                                style={{ width: `${c.cpu}%` }}
                              />
                            </View>
                          </View>
                          <View className="flex-col gap-1">
                            ...
                            <View className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                              <View
                                className={`h-full rounded-full transition-all duration-500 ${c.ram > 80 ? "bg-amber-500" : "bg-indigo-500"}`}
                                style={{ width: `${c.ram}%` }}
                              />
                            </View>
                          </View>
          ...
          <View className="py-2 gap-4">
            <View className="flex-row items-center gap-4 mb-2">
              ...
              <View>
                ...
                <Text className="text-slate-500 text-xs font-mono">
                  {manageNode.region} • {manageNode.uptime} Uptime
                </Text>
              </View>
            </View>
            ...
            <View className="flex-row gap-3 mt-4">
              ...
              <Pressable
                onPress={() => {
                  setManageNode(null);
                  alert("Node scale-up initiated.");
                }}
                className="flex-[2] bg-blue-600 py-3 rounded-lg items-center shadow-md active:bg-blue-700 transition-transform active:scale-95"
              >
