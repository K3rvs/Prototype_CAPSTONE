...
export const StudentHelpFAQ = () => {
  ...
  const FAQS = [
    { id: 1, q: "How do I earn a Soulbound Token (SBT)?", a: "SBTs are awarded by the system for exhibiting positive behaviors, completing major milestones, or participating in communities. Once minted, they are permanently attached to your Academic Passport." },
  ...
  return (
    ...
    <Animated.View className="flex-1 bg-slate-50/50 overflow-hidden flex-col w-full h-full">
      <ScrollView showsVerticalScrollIndicator={true} className="flex-1 w-full" contentContainerClassName="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto w-full pb-20 gap-6 sm:gap-8">
        ...
        <View className="flex-col md:flex-row justify-between md:items-center mb-6 gap-4">
          <View className="flex-1 flex-row items-center gap-4">
            ...
            <View className="flex-1">
              <Text className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Help & FAQ</Text>
              <Text className="text-slate-600 text-sm mt-1 leading-relaxed font-medium">
                Search through our guides, tutorials, and FAQs. If you still need help, reach out to the IT Administration.
              </Text>
            </View>
          </View>
        </View>
        ...
        <View className="flex-col lg:flex-row gap-4 mb-2">
          <View className="flex-1 bg-white flex-row items-center border border-slate-200 rounded-2xl px-4 py-1 h-14 focus-within:border-sky-500 shadow-sm">
            ...
            <TextInput placeholder="Search for help topics or keywords..." className="flex-1 ml-3 text-base text-slate-800 outline-none font-medium h-full" placeholderTextColor="#94a3b8" />
          </View>

          <View className="flex-col sm:flex-row gap-4">
            <Pressable onPress={() => setTicketModal(true)} className="bg-slate-900 px-6 py-4 rounded-2xl shadow-sm active:bg-slate-800 transition-transform active:scale-95 flex-row items-center justify-center gap-3">
              <MaterialCommunityIcons name="ticket-confirmation-outline" size={20} color="white" />
              <Text className="text-white font-bold text-sm">Submit IT Ticket</Text>
            </Pressable>
            <Pressable onPress={() => setChatModal(true)} className="bg-sky-50 border border-sky-200 px-6 py-4 rounded-2xl active:bg-sky-100 transition-transform active:scale-95 flex-row items-center justify-center gap-3 shadow-sm">
              <MaterialCommunityIcons name="chat-processing-outline" size={20} color="#0284c7" />
              <Text className="text-sky-800 font-bold text-sm">Live Chat</Text>
            </Pressable>
          </View>
        </View>
        ...
        <View className="flex-col lg:flex-row gap-6 lg:gap-8">
           ...
           <View className="flex-1 gap-4">
              <Text className="text-xl font-black text-slate-800 px-2 tracking-tight">Video Tutorials</Text>
              <View className="gap-3">
                 {[
                   { title: "Navigating your Dashboard", duration: "2:45", icon: "view-dashboard" },
                   { title: "Taking an AI-generated Quiz", duration: "4:12", icon: "brain" },
                   { title: "Verifying your SBTs", duration: "1:30", icon: "shield-check" }
                 ].map((guide, i) => (
                    <Pressable key={i} className="bg-white p-4 rounded-2xl border border-slate-200 flex-row items-center gap-4 hover:border-sky-300 hover:shadow-md transition-all active:scale-[0.98] group shadow-sm">
                       <View className="w-16 h-12 bg-slate-100 rounded-xl items-center justify-center border border-slate-200 group-hover:bg-sky-50 transition-colors">
                          <MaterialCommunityIcons name="play-circle" size={24} className="text-slate-400 group-hover:text-sky-600 transition-colors" />
                       </View>
                       <View className="flex-1">
                          <Text className="font-bold text-slate-800 text-base mb-0.5 group-hover:text-sky-700 transition-colors">{guide.title}</Text>
                          <Text className="text-slate-500 text-xs font-medium">{guide.duration} • Video Guide</Text>
                       </View>
                       <MaterialCommunityIcons name="chevron-right" size={24} color="#cbd5e1" className="group-hover:text-sky-500 transition-colors" />
                    </Pressable>
                 ))}
              </View>
           </View>
           ...
           <View className="flex-1 lg:flex-[1.5] gap-4">
              <Text className="text-xl font-black text-slate-800 px-2 tracking-tight">Frequently Asked Questions</Text>
              <View className="gap-3">
               {FAQS.map((item, idx) => (
                 <Animated.View key={item.id} className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm hover:border-slate-300 transition-colors">
                    <Pressable onPress={() => setExpanded(expanded === item.id ? null : item.id)} className={`p-5 flex-row items-center justify-between bg-white active:bg-slate-50 transition-colors ${expanded === item.id ? 'border-b border-slate-100' : ''}`}>
                       <Text className={`font-bold text-base flex-1 pr-4 ${expanded === item.id ? 'text-sky-700' : 'text-slate-800'}`}>{item.q}</Text>
                       <View className={`w-8 h-8 rounded-full items-center justify-center transition-colors ${expanded === item.id ? 'bg-sky-50' : 'bg-slate-50'}`}>
                          <MaterialCommunityIcons name={expanded === item.id ? "minus" : "plus"} size={20} color={expanded === item.id ? "#0284c7" : "#64748b"} />
                       </View>
                    </Pressable>
                    {expanded === item.id && (
                      <View className="p-5 bg-slate-50/50">
                        <Text className="text-slate-600 text-sm leading-relaxed font-medium">{item.a}</Text>
                      </View>
                    )}
                 </Animated.View>
               ))}
