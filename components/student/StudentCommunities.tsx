...
import Animated, { FadeIn, FadeInDown, FadeInUp, ZoomIn, useSharedValue, withTiming, useAnimatedScrollHandler, useAnimatedStyle } from "react-native-reanimated";
...
const INITIAL_COMMUNITIES = [
  { id: 1, name: "Science Innovators Club", members: 12, type: "Academic", color: "emerald", icon: "flask" },
  { id: 2, name: "Varsity E-Sports", members: 10, type: "Athletics", color: "rose", icon: "controller-classic" },
  { id: 3, name: "Debate Society", members: 8, type: "Arts & Culture", color: "indigo", icon: "account-voice" },
];

const INITIAL_DISCOVER_CLUBS = [
  { id: 101, name: "Math Wizards", members: 12, type: "Academic", color: "blue", icon: "calculator-variant-outline", desc: "For competitive mathematics training and casual puzzles." },
  { id: 102, name: "Digital Artists", members: 10, type: "Arts", color: "fuchsia", icon: "palette-outline", desc: "Share and critique digital illustrations and graphic design." },
  { id: 103, name: "Eco Warriors", members: 8, type: "Advocacy", color: "lime", icon: "leaf", desc: "Promoting sustainability and organizing campus clean-ups." },
];

const INITIAL_COMMUNITY_CHATS: Record<number, any[]> = {
  1: [ // Science Innovators Club
    { id: 1, user: "Alex (12-STEM)", text: "Hey everyone! Has anyone started on the science fair project?", time: getRelativeTime(45), avatar: "A", color: "blue", reactions: { "👍": 2, "🔥": 1 } },
    { id: 2, user: "Sam (12-STEM)", text: "Yeah, I found some good references. I'll share my notes in the Shared Notes tab.", time: getRelativeTime(40), avatar: "S", color: "emerald", reactions: { "❤️": 3 } },
    { id: 3, user: "Jamie (11-ABM)", text: "Are we allowed to work in pairs for this one?", time: getRelativeTime(33), avatar: "J", color: "amber", reactions: {} },
    { id: 4, user: "Taylor (12-STEM)", text: "Yes, the advisor said pairs or individual are both fine. Deadline is end of the month!", time: getRelativeTime(25), avatar: "T", color: "rose", reactions: { "👍": 5, "💯": 2 } },
    { id: 5, user: "Alex (12-STEM)", text: "Awesome, thanks for clarifying, Taylor!", time: getRelativeTime(20), avatar: "A", color: "blue", reactions: { "🙏": 2 } },
    { id: 6, user: "You", text: "I'm looking for a partner if anyone is interested!", time: getRelativeTime(15), avatar: "Y", color: "sky", reactions: { "👀": 1 } },
    { id: 7, user: "Riley (12-HUMSS)", text: "I'm down! Let's discuss ideas later.", time: getRelativeTime(13), avatar: "R", color: "purple", reactions: { "🙌": 1, "🔥": 1 } },
    { id: 8, user: "Jordan (10-Rizal)", text: "Does anyone have a copy of the rubric? I can't seem to find it in the materials folder.", time: getRelativeTime(10), avatar: "J", color: "teal", reactions: {} },
    { id: 9, user: "Sam (12-STEM)", text: "I just uploaded it to the Shared Notes section! Check the PDF.", time: getRelativeTime(7), avatar: "S", color: "emerald", reactions: { "❤️": 4, "💯": 1 } },
  ],
  2: [ // Varsity E-Sports
    { id: 1, user: "Riley (12-HUMSS)", text: "Who's down for some scrims tonight? The regional tournament is next week!", time: getRelativeTime(50), avatar: "R", color: "purple", reactions: { "🔥": 4 } },
    { id: 2, user: "Casey (11-STEM)", text: "Count me in! I've been studying the enemy team's strategies.", time: getRelativeTime(42), avatar: "C", color: "teal", reactions: { "👍": 2 } },
    { id: 3, user: "Skyler (12-ABM)", text: "I'll join too. Let's make sure our communications are tight.", time: getRelativeTime(30), avatar: "S", color: "blue", reactions: { "💯": 3 } },
    { id: 4, user: "Jordan (11-ABM)", text: "I uploaded our practice quiz on E-Sports Rules & Gameplay. Everyone take a look!", time: getRelativeTime(18), avatar: "J", color: "indigo", reactions: { "👍": 3 } },
  ],
  3: [ // Debate Society
    { id: 1, user: "Avery (11-ABM)", text: "The motion for the next debate round is 'This House would ban homework'. Thoughts?", time: getRelativeTime(60), avatar: "A", color: "pink", reactions: { "😮": 3 } },
    { id: 2, user: "Jamie (11-ABM)", text: "Government side should focus on stress levels and sleep patterns.", time: getRelativeTime(48), avatar: "J", color: "amber", reactions: { "👍": 2 } },
    { id: 3, user: "Robin (12-STEM)", text: "Opposition could argue that homework reinforces learning and discipline.", time: getRelativeTime(36), avatar: "R", color: "green", reactions: { "💡": 4 } },
    { id: 4, user: "Hayden (11-HUMSS)", text: "Great points. Let's do a practice debate round at 4 PM in the lecture hall.", time: getRelativeTime(15), avatar: "H", color: "orange", reactions: { "🔥": 2 } },
  ]
};

const INITIAL_COMMUNITY_NOTES: Record<number, any[]> = {
  1: [ // Science Innovators Club
    { id: 101, title: "Biology Final Reviewer", author: "Sam (12-STEM)", date: new Date(new Date().getTime() - 86400000 * 2).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), upvotes: 24, type: "pdf" },
    { id: 102, title: "Physics Formulas Cheat Sheet", author: "Alex (12-STEM)", date: new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), upvotes: 18, type: "image" },
    { id: 103, title: "Chemistry Lab Guide", author: "Morgan (12-HUMSS)", date: new Date(new Date().getTime() - 86400000 * 10).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), upvotes: 45, type: "presentation" }
  ],
  2: [ // Varsity E-Sports
    { id: 201, title: "MOBA Map Rotation Guide", author: "Casey (11-STEM)", date: new Date(new Date().getTime() - 86400000 * 1).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), upvotes: 35, type: "pdf" },
    { id: 202, title: "E-Sports Strategy Deck", author: "Skyler (12-ABM)", date: new Date(new Date().getTime() - 86400000 * 3).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), upvotes: 52, type: "presentation" }
  ],
  3: [ // Debate Society
    { id: 301, title: "Rhetorical Fallacies List", author: "Avery (11-ABM)", date: new Date(new Date().getTime() - 86400000 * 4).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), upvotes: 19, type: "pdf" },
    { id: 302, title: "Debate Framework Template", author: "Jamie (11-ABM)", date: new Date(new Date().getTime() - 86400000 * 6).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), upvotes: 28, type: "document" }
  ]
};

const INITIAL_COMMUNITY_TESTS = {
  1: [ // Science Innovators Club
    { id: 1, title: "Module 1: Basic Math", type: "Multiple Choice", date: getDateOffset(0), items: 10, color: "rose", author: "Taylor (12-STEM)" },
    { id: 2, title: "Module 2: Primary Science", type: "Identification", date: getDateOffset(0), items: 10, color: "rose", author: "Sam (12-STEM)" },
    { id: 3, title: "Module 3: Shapes and Colors", type: "Enumeration", date: getDateOffset(0), items: 10, color: "rose", author: "Jamie (11-ABM)" },
    { id: 4, title: "Module 4: Reading Comprehension", type: "Mixed", date: getDateOffset(7), items: 10, color: "emerald", author: "Alex (12-STEM)" },
  ],
  2: [ // Varsity E-Sports
    { id: 5, title: "Module 1: E-Sports Regulations & Rules", type: "Multiple Choice", date: getDateOffset(0), items: 10, color: "rose", author: "Jordan (11-ABM)" },
    { id: 6, title: "Module 2: MOBA Strategy & Drafting Quiz", type: "Mixed", date: getDateOffset(0), items: 10, color: "emerald", author: "Skyler (12-ABM)" }
  ],
  3: [ // Debate Society
    { id: 7, title: "Module 1: Rhetoric and Argumentation", type: "Identification", date: getDateOffset(0), items: 10, color: "rose", author: "Avery (11-ABM)" },
    { id: 8, title: "Module 2: Debate Formats & Timing Rules", type: "Enumeration", date: getDateOffset(0), items: 10, color: "emerald", author: "Jamie (11-ABM)" }
  ]
};
...
export const StudentCommunities = () => {
  ...
  const [joinedCommunities, setJoinedCommunities] = useState<any[]>(INITIAL_COMMUNITIES);
  const [discoverClubs, setDiscoverClubs] = useState<any[]>(INITIAL_DISCOVER_CLUBS);

  const [chatMessagesMap, setChatMessagesMap] = useState<Record<number, any[]>>(INITIAL_COMMUNITY_CHATS);
  const [sharedNotesMap, setSharedNotesMap] = useState<Record<number, any[]>>(INITIAL_COMMUNITY_NOTES);
  const [sharedTestsMap, setSharedTestsMap] = useState<Record<number, any[]>>(INITIAL_COMMUNITY_TESTS);
  ...
  const chatMessages = selectedCommunity ? (chatMessagesMap[selectedCommunity.id] || []) : [];
  ...
  const QUESTIONS_MAP: Record<number, any[]> = {
    1: [
      { type: 'Multiple Choice', q: "1. What is 5 + 3?", options: ["6", "7", "8", "9"], ans: "8" },
      { type: 'Multiple Choice', q: "2. Which animal says 'Moo'?", options: ["Dog", "Cow", "Cat", "Bird"], ans: "Cow" },
      { type: 'Multiple Choice', q: "3. What color is the sky on a clear day?", options: ["Green", "Blue", "Red", "Yellow"], ans: "Blue" },
      { type: 'Multiple Choice', q: "4. How many legs does a spider have?", options: ["6", "8", "10", "12"], ans: "8" },
      { type: 'Identification', q: "5. Identification: What do you call a baby dog?", ans: "puppy" },
      { type: 'Identification', q: "6. Identification: What is the opposite of 'Hot'?", ans: "cold" },
      { type: 'Identification', q: "7. Identification: Which planet do we live on?", ans: "earth" },
      { type: 'Enumeration', q: "8. Select all the fruits:", options: ["Apple", "Carrot", "Banana", "Broccoli"], ans: ["Apple", "Banana"] },
      { type: 'Enumeration', q: "9. Select all the colors of the rainbow:", options: ["Red", "Black", "Blue", "Brown"], ans: ["Red", "Blue"] },
      { type: 'Enumeration', q: "10. Select all the farm animals:", options: ["Pig", "Lion", "Chicken", "Tiger"], ans: ["Pig", "Chicken"] }
    ],
    2: [
      { type: 'Multiple Choice', q: "1. What is the capital of France?", options: ["London", "Berlin", "Paris", "Madrid"], ans: "Paris" },
      { type: 'Multiple Choice', q: "2. What is 10 / 2?", options: ["3", "4", "5", "6"], ans: "5" },
      { type: 'Multiple Choice', q: "3. What is the largest mammal?", options: ["Elephant", "Blue Whale", "Giraffe", "Shark"], ans: "Blue Whale" },
      { type: 'Multiple Choice', q: "4. What is the boiling point of water in Celsius?", options: ["50", "90", "100", "120"], ans: "100" },
      { type: 'Multiple Choice', q: "5. A triangle has how many sides?", options: ["2", "3", "4", "5"], ans: "3" },
      { type: 'Multiple Choice', q: "6. Primary source of energy for Earth?", options: ["Moon", "Wind", "Sun", "Geothermal"], ans: "Sun" },
      { type: 'Multiple Choice', q: "7. Fastest land animal?", options: ["Lion", "Cheetah", "Horse", "Leopard"], ans: "Cheetah" },
      { type: 'Multiple Choice', q: "8. How many days in a standard week?", options: ["5", "6", "7", "8"], ans: "7" },
      { type: 'Multiple Choice', q: "9. Opposite of 'Up'?", options: ["Right", "Left", "Down", "Forward"], ans: "Down" },
      { type: 'Multiple Choice', q: "10. Color of a banana?", options: ["Red", "Green", "Yellow", "Blue"], ans: "Yellow" }
    ],
    3: [
      { type: 'Identification', q: "1. Identification: What is H2O commonly known as?", ans: "water" },
      { type: 'Identification', q: "2. Identification: Who painted the Mona Lisa?", ans: "leonardo da vinci" },
      { type: 'Identification', q: "3. Identification: What is the tallest mountain on Earth?", ans: "mount everest" },
      { type: 'Identification', q: "4. Identification: Which planet is known as the Red Planet?", ans: "mars" },
      { type: 'Identification', q: "5. Identification: What language is most spoken in Brazil?", ans: "portuguese" },
      { type: 'Identification', q: "6. Identification: What sweet substance do bees make?", ans: "honey" },
      { type: 'Identification', q: "7. Identification: How many continents are there?", ans: "seven" },
      { type: 'Identification', q: "8. Identification: Name the hard outer covering of a turtle.", ans: "shell" },
      { type: 'Identification', q: "9. Identification: What gas do humans breathe out?", ans: "carbon dioxide" },
      { type: 'Identification', q: "10. Identification: What is the center of an atom called?", ans: "nucleus" }
    ],
    4: [
      { type: 'Enumeration', q: "1. Select all vowels:", options: ["A", "B", "E", "Z"], ans: ["A", "E"] },
      { type: 'Enumeration', q: "2. Select even numbers:", options: ["2", "3", "4", "5"], ans: ["2", "4"] },
      { type: 'Enumeration', q: "3. Select ocean names:", options: ["Pacific", "Nile", "Atlantic", "Sahara"], ans: ["Pacific", "Atlantic"] },
      { type: 'Enumeration', q: "4. Select mammals:", options: ["Dog", "Snake", "Whale", "Lizard"], ans: ["Dog", "Whale"] },
      { type: 'Enumeration', q: "5. Select programming languages:", options: ["Python", "HTML", "Java", "CSS"], ans: ["Python", "Java"] },
      { type: 'Enumeration', q: "6. Select primary colors:", options: ["Red", "Green", "Blue", "Yellow"], ans: ["Red", "Blue", "Yellow"] },
      { type: 'Enumeration', q: "7. Select seasons:", options: ["Winter", "Rain", "Summer", "Cold"], ans: ["Winter", "Summer"] },
      { type: 'Enumeration', q: "8. Select gas giants:", options: ["Jupiter", "Mars", "Saturn", "Earth"], ans: ["Jupiter", "Saturn"] },
      { type: 'Enumeration', q: "9. Select states of matter:", options: ["Solid", "Energy", "Liquid", "Heat"], ans: ["Solid", "Liquid"] },
      { type: 'Enumeration', q: "10. Select Asian countries:", options: ["Japan", "Brazil", "India", "Egypt"], ans: ["Japan", "India"] }
    ]
  };
  ...
  const testPercentage = currentQuestions?.length ? Math.round((testScore / currentQuestions.length) * 100) : 0;
  ...
  const resultMessage = isPerfect ? "Perfect Score! 🏆" : isPassing ? "Quiz Passed! 👍" : "Needs Review. 💪";
  ...
  const getLeaderboard = () => {
    ...
    if (activeTest && (testPhase === 'results' || assessmentAttempts[activeTest.id] !== undefined)) {
  ...
  const filteredAssessments = useMemo(() => {
    ...
    const testList = selectedCommunity ? (sharedTestsMap[selectedCommunity.id] || []) : [];
  ...
  }, [assessTypeFilter, assessDateFilter, customStartDate, customEndDate, dateOffset, selectedCommunity, sharedTestsMap]);
  ...
  const COMMUNITY_MEMBERS = useMemo(() => {
    ...
    const baseList = STUDENT_PEERS.slice(0, baseCount).map((peer, i) => ({
      id: i,
      name: peer.name,
      role: i === 0 ? "Owner" : i === 1 ? "Moderator" : i === 2 ? "Moderator" : "Member",
      avatar: peer.avatar
    }));
  ...
  const handleSendChat = () => {
    ...
    // Safety Interceptor
    const toxicKeywords = ["cheat", "leak", "hack", "steal", "kill", "stupid", "idiot", "bribe", "sell answers", "leak test"];
    const isToxic = toxicKeywords.some(kw => chatText.toLowerCase().includes(kw));
    ...
    if (isToxic) {
      ...
      if (nextWarning === 1) {
        setSafetyModal({
          ...
          message: "Suspicious activity (cheating or harassment language) detected in your chat message. Please maintain academic integrity and a respectful environment. Multiple warnings will lead to an administrative report. (Warning 1/2)",
          level: 1
        });
      } else {
        setSafetyModal({
          title: "🚨 Incident Flagged & Reported",
          message: "You have ignored the safety warning. This suspicious activity has been logged and reported to the school admin portal for counselor intervention.",
          level: 2
        });
      }
      setChatText("");
    ...
    const selectedId = selectedCommunity.id;
    const newMessage = { id: Date.now(), user: "You", text: chatText, time: getRelativeTime(0), avatar: "Y", color: "sky", reactions: {} };
    setChatMessagesMap(prev => ({
      ...prev,
  ...
  const handleInvitePeer = (peer: any) => {
    ...
    setSelectedCommunity((prev: any) => prev ? { ...prev, members: prev.members + 1 } : null);
  };
  ...
  const handleCreateCommunity = () => {
    ...
    const colors = ["emerald", "rose", "indigo", "blue", "fuchsia", "amber", "purple"];
    const randomColor = colors[Math.floor(Math.random() * colors.length)];
    const newComm = {
      id: newId,
      name: newCommName.trim(),
      desc: newCommDesc.trim() || "A community dedicated to peer support and study group collaborations.",
      members: 1,
      type: "Academic",
      color: randomColor,
      icon: "account-group",
    ...
    setChatMessagesMap(prev => ({
      ...prev,
      [newId]: [
        { id: 1, user: "System", text: `Welcome to the new community "${newComm.name}"! Start inviting your peers and sharing resources.`, time: getRelativeTime(0), avatar: "ℹ️", color: "slate", reactions: {} }
      ]
    }));
    ...
    alert(`Community "${newComm.name}" has been proposed and successfully created!`);
  };
  ...
  const handleJoinClub = (club: any) => {
    ...
    setChatMessagesMap(prev => {
      ...
      return {
        ...prev,
        [clubId]: [
          { id: 1, user: "System", text: `Welcome to the "${club.name}" community! Share notes, take practice quizzes, and chat with peers.`, time: getRelativeTime(5), avatar: "ℹ️", color: "slate", reactions: {} }
        ]
      };
    });
    setSharedNotesMap(prev => {
      ...
      return {
        ...prev,
        [clubId]: [
          { id: clubId * 10 + 1, title: `${club.name} Study Resource`, author: "Jamie (11-ABM)", date: new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), upvotes: 12, type: "pdf" }
        ]
      };
    });
    setSharedTestsMap(prev => {
      ...
      return {
        ...prev,
        [clubId]: [
          { id: clubId + 10, title: `${club.name} Warmup Quiz`, type: "Multiple Choice", date: getDateOffset(0), items: 10, color: "emerald", author: "Sam (12-STEM)" }
        ]
      };
    });
  ...
  const handleVerifyInviteCode = () => {
    ...
    if (targetClubId !== -1) {
      ...
      if (club) {
        ...
        const alreadyJoined = joinedCommunities.some(c => c.id === targetClubId);
        if (alreadyJoined) {
  ...
  if (selectedCommunity) {
    const COMMUNITY_TABS = [
      ...
      { id: "notes", label: "Shared Notes", icon: "file-document-multiple-outline" },
      { id: "tests", label: "Practice Quizzes", icon: "clipboard-text-outline" },
    ...
    return (
      ...
      >
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} className="flex-1 flex-col h-full w-full">
          <Animated.View className="flex-1 bg-slate-50/50 overflow-hidden flex-col w-full h-full relative">
            <View className="flex-1 flex-col h-full w-full relative bg-slate-50/50">
              ...
              <View className={`bg-${selectedCommunity.color}-600 p-5 sm:p-6 flex-row items-center justify-between shadow-inner relative overflow-hidden group shrink-0 rounded-2xl`}>
                <MaterialCommunityIcons name={selectedCommunity.icon as any} size={140} color="#ffffff" className="absolute -right-10 -top-10 opacity-10 pointer-events-none" />
                <View className="flex-1 pr-4 z-10">
                  <Pressable onPress={() => setSelectedCommunity(null)} className="self-start flex-row items-center gap-2 bg-white/20 px-3 py-1.5 rounded-lg border border-white/30 mb-4 active:bg-white/30 transition-colors">
                    <MaterialCommunityIcons name="arrow-left" size={16} color="white" />
                    <Text className="text-white font-bold text-xs uppercase tracking-widest">Communities List</Text>
                  </Pressable>
                  <Text className="text-2xl sm:text-3xl font-black text-white tracking-tight mb-2">{selectedCommunity.name}</Text>
                  <View className="flex-row items-center gap-3 flex-wrap">
                    <View className={`bg-${selectedCommunity.color}-700/50 px-3 py-1 rounded border border-${selectedCommunity.color}-500`}>
                      <Text className="text-white font-bold text-xs uppercase tracking-wider">{selectedCommunity.type}</Text>
                    </View>
                    <Text className={`text-${selectedCommunity.color}-100 font-medium text-sm`}>{selectedCommunity.members} Members</Text>
                  </View>
                </View>
              </View>
              ...
              <View className="flex-1 flex-col xl:flex-row p-0 sm:p-0 xl:p-6 gap-0 xl:gap-4 overflow-hidden relative mt-4">
                ...
                {Platform.OS === 'web' && (
                  <View className="hidden xl:flex w-48 flex-col gap-2 shrink-0">
                    <Text className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 px-1">Community Menu</Text>
                    {COMMUNITY_TABS.map(t => (
                      ...
                      >
                        <View className={`w-7 h-7 bg-${selectedCommunity.color}-50 rounded-lg items-center justify-center border border-${selectedCommunity.color}-100`}>
                          <MaterialCommunityIcons name={t.icon as any} size={16} className={`text-${selectedCommunity.color}-600`} />
                        </View>
                        <Text className={`font-bold text-xs sm:text-sm flex-1 ${innerTab === t.id ? `text-${selectedCommunity.color}-800` : 'text-slate-700'}`}>{t.label}</Text>
                      </Pressable>
                    ))}
                  </View>
                )}
                  ...
                  <Animated.View className="flex-1 flex-col relative bg-slate-50/50 sm:rounded-2xl sm:border border-slate-200">
                    <View className="flex-row items-center justify-between px-4 sm:px-6 lg:px-8 py-3 z-10 shrink-0 border-b border-slate-100 bg-white/50 backdrop-">
                      <View className="flex-row items-center gap-2.5">
                        <View className={`w-7 h-7 items-center justify-center bg-${selectedCommunity.color}-100 rounded-full border border-${selectedCommunity.color}-200`}>
                          <MaterialCommunityIcons name="forum-outline" size={14} className={`text-${selectedCommunity.color}-700`} />
                        </View>
                        <View>
                          <Text className="font-bold text-slate-800 text-sm">Community Chat</Text>
                          <Text className={`text-${selectedCommunity.color}-600 text-[9px] font-bold uppercase tracking-widest`}>Online</Text>
                        </View>
                      </View>
                    </View>
                    
                    <Animated.ScrollView ref={chatScrollRef as any} onScroll={scrollHandler} scrollEventThrottle={16} showsVerticalScrollIndicator={false} className="flex-1 w-full bg-transparent" contentContainerClassName="flex-grow py-4 px-4 sm:px-6 md:px-8 items-center pb-8">
                          ...
                          <Animated.View key={msg.id} className={`flex-col w-full mb-2`}>
                            {msg.user === 'You' ? (
                              <View className="self-end max-w-[85%] sm:max-w-[75%] relative">
                                <View className={`bg-${selectedCommunity.color}-600 px-3.5 py-2 rounded-xl rounded-tr-sm shadow-sm z-10`}>
                                  ...
                                  <View className="flex-row justify-end -mt-2 z-20 pr-2">
                                    <View className="flex-row bg-white rounded-full px-1.5 py-0.5 border border-slate-200 shadow-sm">
                                      {Object.entries(msg.reactions).map(([emoji, count]) => (
                                        <Text key={emoji} className="text-[10px] mx-0.5">{emoji} {count as number}</Text>
                                      ))}
                                    </View>
                                  </View>
                                )}
                                <Text className="text-[10px] font-bold text-slate-400 mt-1 text-right">You • {msg.time}</Text>
                              </View>
                            ) : (
                              <View className="self-start max-w-[85%] sm:max-w-[75%] relative group">
                                <View className="flex-row items-center gap-2 mb-1.5 px-1">
                                  <View className={`w-5 h-5 bg-${msg.color}-100 rounded-full items-center justify-center shadow-sm border border-${msg.color}-200`}>
                                    <Text className={`text-[10px] font-bold text-${msg.color}-700`}>{msg.avatar}</Text>
                                  </View>
                                  <Text className={`text-slate-600 font-bold text-[10px]`}>{msg.user}</Text>
                                </View>
                                <View className="flex-row items-center">
                                  ...
                                  {/* Reaction button */}
                                  <Pressable onPress={() => setActiveReactionMsg(activeReactionMsg === msg.id ? null : msg.id)} className="ml-2 w-6 h-6 rounded-full bg-slate-100 items-center justify-center border border-slate-200 opacity-50 hover:opacity-100 transition-opacity hidden sm:flex">
                                    <MaterialCommunityIcons name="emoticon-happy-outline" size={14} color="#64748b" />
                                  </Pressable>
                                </View>
                                {activeReactionMsg === msg.id && (
                                  <View className="absolute top-8 left-10 bg-white rounded-full shadow-lg border border-slate-200 flex-row px-2 py-1 gap-2 z-50">
                                    {QUICK_EMOJIS.map(emoji => (
                                      <Pressable key={emoji} onPress={() => { handleReaction(msg.id, emoji); setActiveReactionMsg(null); }} className="hover:scale-125 transition-transform p-1">
                                ...
                                {msg.reactions && Object.keys(msg.reactions).length > 0 && (
                                  <View className="flex-row justify-start pl-2 -mt-2 z-20">
                                    <View className="flex-row bg-white rounded-full px-1.5 py-0.5 border border-slate-200 shadow-sm">
                                      {Object.entries(msg.reactions).map(([emoji, count]) => (
                                        <Pressable key={emoji} onPress={() => handleReaction(msg.id, emoji)}>
                                          <Text className="text-[10px] mx-0.5">{emoji} {count as number}</Text>
                                        </Pressable>
                                      ))}
                                    </View>
                                  </View>
                                )}
                                <Text className="text-[10px] font-bold text-slate-400 mt-1 pl-1">{msg.time}</Text>
                              </View>
                            )}
                          </Animated.View>
                        ))}
                    ...
                    <View className={`w-full px-4 pt-2 bg-transparent flex-row justify-center ${Platform.OS === 'web' ? 'pb-24 xl:pb-6' : 'pb-0'}`}>
                      <View className={`w-full flex-col items-center ${Platform.OS === 'web' ? 'max-w-5xl' : 'max-w-3xl'}`}>
                         <View className={`bg-white/80 rounded-2xl border border-slate-200/80 focus-within:bg-white focus-within:border-${selectedCommunity.color}-400 focus-within:shadow-xl transition-all shadow-sm flex-row items-end px-2 py-1.5 w-full gap-2`}>
                          ...
                          />
                           <Pressable onPress={handleSendChat} disabled={!chatText.trim()} className={`h-8 w-8 rounded-full items-center justify-center transition-all active:scale-95 ${chatText.trim() ? `bg-${selectedCommunity.color}-600 hover:bg-${selectedCommunity.color}-700 shadow-md shadow-${selectedCommunity.color}-500/30` : 'bg-slate-300'}`}>
                             <MaterialCommunityIcons name="send" size={18} color={chatText.trim() ? "white" : "#94a3b8"}/>
                           </Pressable>
                         </View>
                         <Text className="text-center text-slate-500 text-[10px] mt-1.5 font-medium px-4 mb-4">Messages are visible to all members of {selectedCommunity.name}.</Text>
                      </View>
                    </View>
                  </Animated.View>
             )}
             {innerTab === "notes" && (
                    <Animated.ScrollView onScroll={scrollHandler} scrollEventThrottle={16} className="flex-1 px-4 sm:px-6 lg:px-0" contentContainerClassName="pb-24" showsVerticalScrollIndicator={true}>
                      <Animated.View className="gap-3">
                        <View className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm z-10 flex-col justify-center mb-2">
                          <View className="flex-row justify-between items-center mb-1">
                            <Text className="text-lg sm:text-xl font-black text-slate-800 tracking-tight">Community Notes</Text>
                          <Pressable onPress={() => setUploadModal(true)} className={`bg-${selectedCommunity.color}-600 px-4 py-2.5 rounded-xl border border-${selectedCommunity.color}-500 flex-row items-center gap-2 active:scale-95 transition-transform shadow-sm`}>
                            <MaterialCommunityIcons name="upload" size={16} color="white" />
                              <Text className="text-white font-bold text-sm hidden sm:flex">Upload Note</Text>
                          </Pressable>
                          </View>
                          <Text className="text-slate-500 text-xs sm:text-sm font-medium">Shared resources by members.</Text>
                        </View>
                        {(sharedNotesMap[selectedCommunity.id] || []).map(note => (
                          <View key={note.id} className="w-full bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-slate-200 flex-col sm:flex-row sm:items-center justify-between gap-3 hover:shadow-md transition-shadow">
                            <View className="flex-row items-center gap-3 flex-1">
                              <View className={`w-10 h-10 bg-${selectedCommunity.color}-50 rounded-lg items-center justify-center border border-${selectedCommunity.color}-100`}>
                                <MaterialCommunityIcons name={note.type === 'pdf' ? 'file-pdf-box' : note.type === 'document' ? 'file-word-box' : note.type === 'presentation' ? 'file-powerpoint-box' : 'image'} size={20} className={`text-${selectedCommunity.color}-600`} />
                              </View>
                              <View className="flex-1">
                                <Text className="font-bold text-slate-800 text-sm leading-tight mb-0.5">{note.title}</Text>
                                <Text className="text-slate-500 text-[10px] sm:text-xs font-medium uppercase tracking-wider">By {note.author} • {note.date}</Text>
                              </View>
                            </View>
                            <View className="flex-row items-center justify-between sm:justify-end gap-3 self-start sm:self-auto w-full sm:w-auto mt-2 sm:mt-0 pt-2 sm:pt-0 border-t border-slate-100 sm:border-t-0">
                              ...
                              <Pressable className={`py-1.5 px-3 bg-slate-50 rounded-md border border-slate-200 active:bg-slate-100 transition-colors shadow-sm flex-row items-center justify-center gap-1.5`}>
                                ...
                                <Text className="font-bold text-slate-600 text-xs hidden sm:flex">Download</Text>
                              </Pressable>
                            </View>
                          </View>
                        ))}
                    ...
                    <Animated.ScrollView onScroll={scrollHandler} scrollEventThrottle={16} className="flex-1 px-4 sm:px-6 lg:px-0" contentContainerClassName="pb-24" showsVerticalScrollIndicator={true}>
                      <Animated.View className="flex-1 flex-col pb-8 relative z-20">
                        ...
                        <View className="flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-3 z-50 relative bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm">
                          <View>
                            <Text className="text-lg sm:text-xl font-black text-slate-800">Practice Quizzes</Text>
                            <Text className="text-xs sm:text-sm text-slate-500 font-medium mt-1">Tests created by community members to help you practice.</Text>
                          </View>
                        ...
                        <View className="gap-3">
                           {filteredAssessments.length === 0 && (
                              <Text className="text-center text-slate-500 py-10 font-medium">No practice quizzes match your filters.</Text>
                           )}
                           {filteredAssessments.map((a: any) => {
                             const isCompleted = assessmentAttempts[a.id] !== undefined;
                             const highestScore = assessmentHighestScores[a.id];
                             return (
                               <Pressable key={a.id} onPress={() => handleAssessmentClick(a)} className={`bg-white p-3 sm:p-4 rounded-xl shadow-sm border flex-col sm:flex-row sm:items-center justify-between gap-3 transition-transform active:scale-[0.98] ${isCompleted ? 'border-emerald-200' : 'border-slate-200'}`}>
                                 <View className="flex-row items-center gap-3 flex-1">
                                   <View className={`w-8 h-8 rounded-lg items-center justify-center border ${isCompleted ? 'bg-emerald-50 border-emerald-100' : 'bg-slate-50 border-slate-100'}`}>
                                     <MaterialCommunityIcons name={isCompleted ? 'check-decagram' : 'book-open-variant'} size={16} color={isCompleted ? '#059669' : '#64748b'} />
                                   </View>
                                   <View className="flex-1">
                                     <Text className="font-bold text-slate-800 text-sm mb-0.5">{a.title}</Text>
                                     <Text className="text-slate-500 text-[10px] sm:text-xs font-medium">By {a.author} • {a.type} • {a.items} Items • {a.date}</Text>
                                   </View>
                                 </View>
                                   ...
                                   {isCompleted && (
                                     <>
                                       <View className="px-2 py-1 rounded border bg-emerald-100 border-emerald-200">
                                         <Text className="text-[9px] font-black uppercase tracking-widest text-emerald-700">Completed</Text>
                                       </View>
                                       <Text className="text-xl font-black text-emerald-700">
                                          {highestScore !== undefined ? `${highestScore}/${a.items}` : `--/${a.items}`}
                                       </Text>
                                     </>
                                   )}
                                 </View>
                    ...
                    <Animated.ScrollView onScroll={scrollHandler} scrollEventThrottle={16} className="flex-1 px-4 sm:px-6 lg:px-0" contentContainerClassName="pb-24" showsVerticalScrollIndicator={true}>
                      <Animated.View className="gap-3 pb-6">
                        <View className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm z-10 flex-row justify-between items-center mb-2">
                          <View>
                            <Text className="text-lg sm:text-xl font-black text-slate-800 tracking-tight">Community Roster</Text>
                            <Text className="text-slate-500 text-xs sm:text-sm mt-0.5 font-medium">{COMMUNITY_MEMBERS.length} Active Members</Text>
                          </View>
                          <Pressable onPress={() => setInviteModalOpen(true)} className={`bg-${selectedCommunity.color}-600 px-3.5 py-2 rounded-xl active:bg-${selectedCommunity.color}-700 shadow-sm transition-transform active:scale-95 flex-row items-center gap-1.5`}>
                            <MaterialCommunityIcons name="account-plus" size={14} color="white" />
                            <Text className="text-white font-bold text-xs">Invite</Text>
                          </Pressable>
                        </View>
                        <View className="flex-col gap-2">
                          {COMMUNITY_MEMBERS.map((peer) => (
                            <Pressable key={peer.id} onPress={() => setPeerModal(peer)} className={`w-full bg-white border border-slate-200 p-3 sm:p-4 rounded-xl shadow-sm flex-row items-center justify-between gap-3 hover:border-${selectedCommunity.color}-300 hover:shadow-md transition-all active:scale-[0.98]`}>
                              <View className="flex-row items-center gap-3 flex-1">
                                <View className={`w-10 h-10 rounded-full items-center justify-center bg-${selectedCommunity.color}-50 border border-${selectedCommunity.color}-100 shadow-sm`}>
                                  <Text className={`font-black text-${selectedCommunity.color}-700 text-base`}>{peer.avatar}</Text>
                                </View>
                                <View className="flex-1">
                  ...
                  {COMMUNITY_TABS.map(t => (
                    ...
                    >
                      <MaterialCommunityIcons 
                        name={t.icon as any} 
                        size={24} 
                        className={innerTab === t.id ? `text-${selectedCommunity.color}-600` : 'text-slate-500'} 
                      />
                      <Text className={`text-[10px] font-bold text-center ${innerTab === t.id ? `text-${selectedCommunity.color}-700` : 'text-slate-500'}`} numberOfLines={1}>{t.label}</Text>
                    </Pressable>
                  ))}
                </Animated.View>
              )}
                    ...
                    return (
                      ...
                      >
                        <MaterialCommunityIcons 
                          name={t.icon as any} 
                          size={24} 
                          className={isActive ? `text-${selectedCommunity.color}-700` : "text-slate-500"} 
                        />
                        <Text className={`text-[10px] font-bold mt-1 text-center ${isActive ? `text-${selectedCommunity.color}-800` : "text-slate-500"}`} numberOfLines={1}>
                          {t.id === 'chat' ? 'Chat' : t.id === 'notes' ? 'Notes' : t.id === 'tests' ? 'Tests' : 'Members'}
                        </Text>
                      </Pressable>
                    );
                  })}
                 ...
                 <View className="flex-1 items-center max-w-3xl mx-auto w-full py-4">
                    ...
                    <View className="w-full flex-col sm:flex-row gap-3 max-w-lg mx-auto">
                       <Pressable onPress={() => { setTestPhase('none'); setActiveTest(null); }} className="flex-1 bg-slate-200 py-2.5 rounded-xl items-center active:bg-slate-300 transition-colors">
                          <Text className="font-bold text-slate-700 text-xs">Back to Quizzes</Text>
                       </Pressable>
        ...
        <GlobalModal isOpen={nicknameModal} onClose={() => setNicknameModal(false)} title="Enter Nickname">
         ...
         <GlobalModal isOpen={uploadModal} onClose={() => setUploadModal(false)} title="Upload Study Material">
            <View className="py-2 gap-4 mb-6">
               <Text className="text-slate-600 font-medium px-1">Share lecture guides, reviewer notes, or homework reference sheets with your peers.</Text>
               <View>
                 <Text className="font-bold text-slate-700 mb-2">Note Title</Text>
                 <TextInput
                   value={noteTitle}
            ...
            <View className="flex-row gap-3">
              <Pressable onPress={() => setUploadModal(false)} className="flex-1 bg-slate-100 py-3.5 rounded-xl items-center border border-slate-200 active:bg-slate-200 transition-colors">
              ...
              >
                <Text className="text-white font-bold text-sm">Share to Community</Text>
              </Pressable>
            </View>
         </GlobalModal>

          <GlobalModal isOpen={confirmStartModal} onClose={() => setConfirmStartModal(false)} title="Practice Quiz Verification">
             <View className="items-center py-6">
                ...
                <Text className="text-slate-600 text-center px-6 mb-8 leading-relaxed font-medium">Once you start the practice quiz, you can submit your score to the community leaderboard.</Text>
                <View className="flex-row gap-3 w-full">
          ...
          <GlobalModal isOpen={!!upcomingModal} onClose={() => setUpcomingModal(null)} title="Scheduled Quiz">
             {upcomingModal && (
          ...
          <GlobalModal isOpen={customDateModal} onClose={() => setCustomDateModal(false)} title="Select Date Range">
             <View className="py-2 gap-4 w-full">
                   ...
                   <View className="flex-row flex-wrap p-1">
                      {Array.from({ length: 42 }).map((_, i) => {
                        ...
                        <Pressable key={i} disabled={!valid} onPress={() => valid && handleDayPress(dateNum)} className={`p-1 sm:p-1.5 h-10 sm:h-12 items-center justify-center`} style={{ width: '14.28%' }}>
                            {valid && (
                              <View className={`w-full h-full items-center justify-center rounded-lg ${isStartOrEnd ? `bg-${selectedCommunity?.color || 'sky'}-600 shadow-md shadow-${selectedCommunity?.color || 'sky'}-500/30` : inRange ? `bg-${selectedCommunity?.color || 'sky'}-100` : 'bg-transparent hover:bg-slate-100'}`}>
                                <Text className={`text-xs sm:text-sm font-bold ${isStartOrEnd ? 'text-white' : inRange ? `text-${selectedCommunity?.color || 'sky'}-800` : 'text-slate-700'}`}>{dateNum}</Text>
                              </View>
                            )}
                          </Pressable>
                        );
                ...
                <View className="flex-row gap-3 mt-2 w-full">
                  ...
                  <Pressable onPress={() => { setCustomStartDate(tempStart); setCustomEndDate(tempEnd); setAssessDateFilter('Custom'); setCustomDateModal(false); }} disabled={!tempStart || !tempEnd} className={`flex-[2] py-4 rounded-xl items-center shadow-md active:scale-95 transition-transform ${tempStart && tempEnd ? `bg-${selectedCommunity?.color || 'sky'}-600 hover:bg-${selectedCommunity?.color || 'sky'}-500 active:bg-${selectedCommunity?.color || 'sky'}-700 shadow-${selectedCommunity?.color || 'sky'}-500/30` : 'bg-slate-300'}`}>
          ...
          <GlobalModal isOpen={inviteModalOpen} onClose={() => setInviteModalOpen(false)} title="Invite Student Peers">
            <View className="py-2 gap-4">
              ...
              <ScrollView className="max-h-80" showsVerticalScrollIndicator={true}>
                <View className="gap-2">
                  {(() => {
                    ...
                    const nonMembers = STUDENT_PEERS.filter(p => !memberNames.includes(p.name));
                    
                    if (nonMembers.length === 0) {
                      return <Text className="text-slate-500 text-center py-4 font-medium">All peers are already members.</Text>;
                    }
                    
                    return nonMembers.map((peer, idx) => (
                      <View key={idx} className="flex-row items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-xl">
                        <View className="flex-row items-center gap-3">
                          <View className={`w-8 h-8 rounded-full items-center justify-center bg-${selectedCommunity?.color || 'sky'}-100`}>
                            <Text className={`font-black text-${selectedCommunity?.color || 'sky'}-700 text-sm`}>{peer.avatar}</Text>
                          </View>
                          <Text className="font-bold text-slate-800 text-sm">{peer.name}</Text>
                        </View>
                        <Pressable 
                          onPress={() => {
              ...
              <Pressable onPress={() => setInviteModalOpen(false)} className="w-full bg-slate-800 py-3.5 rounded-xl items-center active:bg-slate-900 transition-colors mt-2">
                <Text className="text-white font-bold text-base">Close</Text>
              </Pressable>
            </View>
          </GlobalModal>

          <GlobalModal isOpen={!!safetyModal} onClose={() => setSafetyModal(null)} title={safetyModal?.title || "Safety Notification"}>
            <View className="items-center py-6">
              <View className={`w-20 h-20 rounded-full items-center justify-center mb-6 border-[6px] ${safetyModal?.level === 1 ? 'bg-amber-50 border-amber-100' : 'bg-red-50 border-red-100'}`}>
                <MaterialCommunityIcons 
                  name={safetyModal?.level === 1 ? "alert-outline" : "alert-decagram"} 
                  size={36} 
              ...
              >
                <Text className="text-white font-bold text-base">Acknowledge</Text>
              </Pressable>
            </View>
          </GlobalModal>
  ...
  return (
    ...
    <Animated.View className="flex-1 bg-slate-50/50 overflow-hidden flex-col w-full h-full">
      ...
      <ScrollView showsVerticalScrollIndicator={true} className="flex-1 bg-slate-50/50 p-4 sm:p-6">
        <View className="flex-col md:flex-row justify-between md:items-center mb-6 gap-4">
          <View className="flex-1 flex-row items-center gap-4">
            <View className="w-12 h-12 bg-emerald-100 rounded-xl items-center justify-center border border-emerald-200 shadow-sm">
              <MaterialCommunityIcons name="account-group-outline" size={24} color="#059669" />
            </View>
            <View className="flex-1">
              <Text className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Community</Text>
          ...
          <View className="flex-col sm:flex-row gap-3">
            ...
            >
              <MaterialCommunityIcons name="magnify" size={18} color="#059669" />
              <Text className="text-emerald-700 font-bold text-sm">Join Club</Text>
            </Pressable>
            <Pressable
            ...
            >
              ...
              <Text className="text-white font-bold text-sm">Propose New</Text>
            </Pressable>
          </View>
        </View>
          ...
          <Animated.View className="flex-row flex-wrap gap-4 sm:gap-6 pb-8">
            {joinedCommunities.map((club, i) => (
              ...
              >
                <Pressable onPress={() => setSelectedCommunity(club)} className={`bg-white rounded-3xl shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all overflow-hidden border border-slate-100 active:scale-[0.97] flex-col min-h-[220px] group hover:border-${club.color}-300 relative`}>
                  {Platform.OS === 'web' && <View className="absolute inset-0 bg-gradient-to-br from-white to-slate-50 opacity-50 pointer-events-none" />}
                  {/* Top colored part */}
                  <View className={`bg-${club.color}-600 p-5 relative h-32 flex-col justify-between overflow-hidden`}>
                    <View className={`absolute inset-0 bg-gradient-to-br from-${club.color}-500 to-${club.color}-700`} />
                    ...
                    <MaterialCommunityIcons name={club.icon as any} size={110} className="absolute -right-4 -bottom-8 text-white/20 transition-transform duration-700 group-hover:scale-110 group-hover:-rotate-12" />
                    <MaterialCommunityIcons name="account-group-outline" size={40} className="absolute left-1/2 top-4 text-white/10 -translate-x-1/2" />
                    ...
                    <View className="z-10 h-full flex-col justify-between">
                      <View className="flex-row items-start justify-between">
                        <View className="bg-white/20 px-2.5 py-1 rounded-md border border-white/30 shadow-sm backdrop-">
                          <Text className="text-white text-[9px] font-black uppercase tracking-widest">{club.type}</Text>
                        </View>
                      ...
                      <Text className="text-white font-black text-lg sm:text-xl leading-tight tracking-tight shadow-sm mt-2" numberOfLines={2}>{club.name}</Text>
                    </View>
                  </View>
                  {/* Bottom white part */}
                  <View className="p-5 flex-1 flex-col justify-between bg-white relative z-10">
                    <View className="absolute right-5 -top-10 w-20 h-20 bg-white rounded-2xl items-center justify-center border-4 border-white shadow-md z-20">
                      <MaterialCommunityIcons name="account-group" size={36} className={`text-${club.color}-600`} />
                    </View>
                    
                    <View className="mt-1 pr-14">
                      <Text className="text-slate-800 text-base font-black mb-2">Community Hub</Text>
                      <View className={`bg-${club.color}-50/50 px-2.5 py-1.5 rounded-lg border border-${club.color}-100/50 self-start flex-row items-center gap-1.5`}>
                        <MaterialCommunityIcons name="account-multiple" size={12} className={`text-${club.color}-600`} />
                        <Text className={`text-${club.color}-700 text-[10px] font-bold`}>{club.members} Members</Text>
                      </View>
                    </View>
                    ...
                    <View className="flex-row items-center justify-between mt-4 pt-3 border-t border-dashed border-slate-200">
                      <View className="flex-row items-center gap-2">
                        ...
                        <Text className="text-slate-500 text-[9px] font-bold uppercase tracking-wider">Public Access</Text>
                      </View>
                      <View className={`w-8 h-8 rounded-full bg-${club.color}-50 items-center justify-center group-hover:bg-${club.color}-600 transition-colors`}>
                        <MaterialCommunityIcons name="arrow-right" size={16} className={`text-${club.color}-600 group-hover:text-white`} />
                      </View>
                    </View>
                  </View>
                </Pressable>
      ...
      <GlobalModal isOpen={joinModal} onClose={() => setJoinModal(false)} title="Join a Community">
        <View className="py-2 gap-4">
           <Text className="text-slate-600 font-medium leading-relaxed">
             Enter an invite code or browse the public directory to join new organizations.
           </Text>
           ...
           <Text className="font-black text-slate-800 text-sm mb-1">Public Directory</Text>
           <ScrollView className="max-h-60" showsVerticalScrollIndicator={true}>
             <View className="gap-3">
               {discoverClubs.length === 0 ? (
                 <Text className="text-center text-slate-500 py-4 font-medium">No new clubs to discover.</Text>
               ) : (
                 discoverClubs.map((club) => (
                   <View key={club.id} className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200 flex-col gap-2">
                     <View className="flex-row items-center justify-between">
                       <View className="flex-row items-center gap-2.5">
                         <View className={`w-8 h-8 rounded-lg bg-${club.color}-100 items-center justify-center`}>
                           <MaterialCommunityIcons name={club.icon as any} size={16} className={`text-${club.color}-700`} />
                         </View>
                         <View>
                           <Text className="font-bold text-slate-800 text-xs sm:text-sm">{club.name}</Text>
                           <Text className="text-slate-500 text-[10px] font-medium">{club.members} members</Text>
                         </View>
                       </View>
                       <Pressable 
                         onPress={() => {
           ...
           <Pressable onPress={() => setJoinModal(false)} className="w-full bg-slate-800 py-3.5 rounded-xl items-center active:bg-slate-900 transition-colors mt-2">
              <Text className="text-white font-bold text-base">Close</Text>
           </Pressable>
        </View>
      </GlobalModal>
      ...
      <GlobalModal isOpen={createCommunityModal} onClose={() => setCreateCommunityModal(false)} title="Propose New Community">
        ...
        <View className="flex-row gap-3 w-full">
          ...
          <Pressable onPress={handleCreateCommunity} className="flex-[2] bg-sky-600 py-4.5 rounded-2xl items-center shadow-md shadow-sky-500/30 active:bg-sky-700 transition-transform active:scale-95">
             <Text className="text-white font-bold text-base">Submit Proposal</Text>
          </Pressable>
        </View>
      </GlobalModal>

      <GlobalModal isOpen={!!peerModal} onClose={() => setPeerModal(null)} title="Member Profile">
         {peerModal && (
           <View className="items-center py-6">
              <View className={`w-24 h-24 bg-${selectedCommunity?.color || 'sky'}-100 rounded-[2rem] border-4 border-white shadow-md items-center justify-center mb-4`}>
                 <Text className={`text-3xl font-black text-${selectedCommunity?.color || 'sky'}-700`}>{peerModal.avatar}</Text>
              </View>
              <Text className="text-2xl font-black text-slate-800 tracking-tight mb-1">{peerModal.name}</Text>
              <View className={`bg-${selectedCommunity?.color || 'sky'}-50 border border-${selectedCommunity?.color || 'sky'}-200 px-3 py-1 rounded-full mb-8`}>
                 <Text className={`text-${selectedCommunity?.color || 'sky'}-700 font-bold text-[10px] uppercase tracking-widest`}>{peerModal.role}</Text>
              </View>
