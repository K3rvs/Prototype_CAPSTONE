...
const TIMELINE_COLOR_CLASSES: Record<
  string,
  {
    borderLeft: string;
    bg: string;
    ...
    return Array.from({ length: 35 }).map((_, i) => {
      ...
      if (isValidDate) {
        ...
        if (y === 2026) {
          if (m === 5) {
            if (d >= 8 && d <= 11)
              dayEvents.push({ label: "Opening Block", color: "blue" });
            if (d === 12)
              dayEvents.push({ label: "Independence Day", color: "rose" });
            if (d >= 15)
              dayEvents.push({ label: "Instructional", color: "blue" });
            if (d === 20)
              dayEvents.push({ label: "Chapter 3 Defense", color: "orange" });
            if (d === 22)
              dayEvents.push({ label: "Math Set 4", color: "purple" });
            if (d === 25)
              dayEvents.push({ label: "Literature Output", color: "purple" });
          } else if (m === 7) {
            if (d === 21)
              dayEvents.push({ label: "Ninoy Aquino Day", color: "rose" });
            if (d === 31)
              dayEvents.push({ label: "Heroes Day", color: "rose" });
          } else if (m === 8) {
            if (d >= 2 && d <= 15)
              dayEvents.push({ label: "End-of-Term", color: "emerald" });
            if (d >= 16)
              dayEvents.push({ label: "Instructional", color: "blue" });
          } else if (m === 10) {
            if (d === 1 || d === 2)
              dayEvents.push({ label: "All Saints/Souls", color: "rose" });
            if (d === 30)
              dayEvents.push({ label: "Bonifacio Day", color: "rose" });
          } else if (m === 11) {
            if (d >= 7 && d <= 18)
              dayEvents.push({ label: "End-of-Term", color: "emerald" });
            if (d === 8)
              dayEvents.push({ label: "Immaculate Conception", color: "rose" });
            if (d >= 19)
              dayEvents.push({ label: "Holiday Break", color: "amber" });
          } else {
            if (m === 6 || m === 9)
              dayEvents.push({ label: "Instructional", color: "blue" });
          }
          ...
          if (m === 0) {
            ...
            if (d === 25)
              dayEvents.push({ label: "EDSA Anniversary", color: "rose" });
            ...
            if (d >= 24)
              dayEvents.push({ label: "End-of-Term", color: "emerald" });
            if (d === 25 || d === 26)
              dayEvents.push({ label: "Holy Week", color: "rose" });
          } else if (m === 3) {
            if (d <= 8)
              dayEvents.push({ label: "End-of-Term", color: "emerald" });
          }
          ...
          <View className="flex-1 w-full flex-col min-h-[500px] lg:min-h-[600px]">
              ...
              <View className="flex-1 w-full lg:flex-[2] flex-col">
                <View className="flex-col sm:flex-row sm:items-center justify-between mb-4 gap-4">
                  ...
                  <View className="flex-row gap-2 bg-slate-50 p-1.5 rounded-xl border border-slate-200 shadow-sm self-start sm:self-auto">
                    <Pressable
                      ...
                      className="px-4 py-2 bg-white rounded-lg   transition-colors shadow-sm border border-slate-200/50 items-center justify-center"
                    >
                    ...
                    <Pressable
                      ...
                      className="p-2 bg-white rounded-lg   transition-colors shadow-sm border border-slate-200/50 items-center justify-center"
                    >
                      <MaterialCommunityIcons
                ...
                <View className="mb-4 w-full -mx-4 px-4 sm:mx-0 sm:px-0">
                  <ScrollView
                    ...
                    contentContainerClassName="flex-row gap-2 pr-8 sm:pr-0 pb-2"
                  >
                    {MONTHS.map((m, idx) => (
                      <Pressable
                        key={m}
                      ...
                      >
                        <Text
                          className={`font-bold text-xs sm:text-sm ${calMonth === idx ? "text-white" : "text-slate-600"}`}
                        >
                          {m.substring(0, 3)}
                        </Text>
                      </Pressable>
                  ...
                  <View className="flex-row flex-wrap flex-1">
                    {calendarDays.map((cell) => {
                      ...
                      return (
                        ...
                        >
                          <View
                            className={`w-full h-full rounded-md sm:rounded-xl p-1 sm:p-2 transition-all ${!isValidDate ? "bg-transparent" : isToday ? "bg-sky-500 shadow-md shadow-sky-500/30" : "bg-slate-50   border border-slate-100/50"}`}
                          >
                            {isValidDate && (
                              <>
                                <View className="flex-row justify-between items-start mb-1">
                                  <Text
                                    className={`text-xs sm:text-sm font-bold ${isToday ? "text-white" : "text-slate-700"}`}
                                  >
                                    {date}
                                  </Text>
                                </View>
                                ...
                                {Platform.OS === "web" ? (
                                  <View className="flex-col gap-1 overflow-hidden">
                                    {dayEvents.slice(0, 3).map((ev, eIdx) => {
                                      const colors =
                                        CALENDAR_COLOR_CLASSES[ev.color] ||
                                        CALENDAR_COLOR_CLASSES.blue;
                                      return (
                                        <View
                                          key={eIdx}
                                          className={`border rounded-md px-1.5 py-0.5 overflow-hidden shadow-sm ${isToday ? "bg-white/20 border-white/30" : `${colors.bg} ${colors.border}`}`}
                                        >
                                          <Text
                                            className={`text-[8px] sm:text-[9px] font-bold leading-tight ${isToday ? "text-white" : colors.text}`}
                                            numberOfLines={1}
                                            ellipsizeMode="tail"
                                            style={{ flexShrink: 1 }}
                                          >
                                    ...
                                    {dayEvents.length > 3 && (
                                      <Text
                                        className={`text-[9px] ${isToday ? "text-sky-100" : "text-slate-400"} font-bold ml-0.5 mt-0.5`}
                                      >
                                  ...
                                  <View className="flex-row gap-1 justify-center mt-1 flex-wrap">
                                    {dayEvents.map((ev, eIdx) => {
                                      ...
                                      return (
                                        <View
                                          key={eIdx}
                                          className={`w-1.5 h-1.5 rounded-full ${isToday ? colors.dotBgToday : colors.dotBg}`}
                                        />
                                      );
                                    })}
                                  </View>
                  ...
                  >
                    <View className="gap-0 pr-1">
                      {ALL_EVENTS.map((event, index) => {
                        const colors =
                          TIMELINE_COLOR_CLASSES[event.color] ||
                          TIMELINE_COLOR_CLASSES.blue;
                        return (
                          <View key={event.id}>
                            <Pressable
                            ...
                            >
                              <View
                                className={`w-10 h-10 rounded-full items-center justify-center border ${colors.bg} ${colors.border}`}
                              >
                                <MaterialCommunityIcons
                                  name="calendar-clock"
                                  size={20}
            ...
            >
              {eventModal &&
                (() => {
                  const colors =
                    CALENDAR_COLOR_CLASSES[eventModal.color] ||
                    CALENDAR_COLOR_CLASSES.blue;
                  const timelineColors =
                    TIMELINE_COLOR_CLASSES[eventModal.color] ||
                    TIMELINE_COLOR_CLASSES.blue;
                  return (
                    <View className="mb-2 mt-2">
                      <View className="items-center">
                        <View
                          className={`w-24 h-24 rounded-[2rem] items-center justify-center mb-4 border-[6px] shadow-sm ${colors.modalBg} ${colors.modalBorder}`}
                        >
                          <MaterialCommunityIcons
                            name="calendar-star"
                            size={40}
                            color={timelineColors.iconColor}
                          />
                        </View>
                        ...
                        <View
                          className={`px-3 py-1.5 rounded-full mb-6 border shadow-sm ${colors.badgeBg} ${colors.badgeBorder}`}
                        >
                          <Text
                            className={`text-[10px] font-black uppercase tracking-widest ${colors.badgeText}`}
                          >
                            {eventModal.type}
                          </Text>
                        </View>
                      ...
                      <View className="flex-row gap-3 w-full">
                        ...
                        <Pressable
                          onPress={() => {
                            setEventModal(null);
                            Alert.alert("Reminder set for this event!");
                          }}
                          className={`flex-[2] py-4.5 rounded-2xl items-center shadow-md transition-transform flex-row justify-center gap-2 ${colors.dotBg} shadow-sm active:scale-95`}
                        >
                          <MaterialCommunityIcons
